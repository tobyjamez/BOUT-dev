 ...
 *
 *     int other;
 *     options.get("otherkey", other, 2.0); // Sets other to 2.0 because "otherkey" not
 * found
 *
...
public:
  ...
  Options(Options *parent, string sectionName)
      : parent(parent), sectionName(sectionName){};
...
#define OPTION(options, var, def) options->get(#var, var, def)
