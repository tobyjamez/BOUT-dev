...
PetscErrorCode advanceStepWrapper(Mat matOperator, Vec inData, Vec outData) {
  PetscFunctionBegin;
  SlepcSolver *ctx;
  MatShellGetContext(matOperator,
                     (void **)&ctx); // Here we set the ctx pointer to the solver instance
...
// A simple wrapper around the SlepcSolver compareEigs routine
PetscErrorCode monitorWrapper(EPS eps, PetscInt its, PetscInt nconv, PetscScalar *eigr,
                              PetscScalar *eigi, PetscReal *errest, PetscInt nest,
                              void *mctx) {
  PetscFunctionBegin;
  // Cast context as SlepcSolver and call the actual compare routine
  SlepcSolver *myCtx;
...
SlepcSolver::SlepcSolver(Options *options) {
  ...
  options->get("targIm", targIm,
               0.0); // Target growth rate when using user eig comparison
  ...
  options->get("target", target,
               target); // If 999 we don't set the target. This is SLEPc eig target
...
void SlepcSolver::createShellMat() {
  ...
  MatCreateShell(comm, localSize, localSize, PETSC_DETERMINE, PETSC_DETERMINE, this,
                 &shellMat); // Note we pass the this reference as the matrix context.
...
// the current
// status of the run.
// Note we could see how many new eigenpairs have been found since last called and then
// write their
// data to file so that we get progressive output rather than waiting until the end to
// write everything.
// Unfortunately it seems that currently SLEPc does not support using EPSGetEigenvector or
// EPSGetEigenpair before EPSSolve has finished. As such it's not possible to write out
// the eigenvectors
// from this monitor routine. It should still be possible to write the eigenvalues here,
// but to then
// get the eigenvectors at the correct time indices later would require resetting the time
// index. I'm
...
void SlepcSolver::analyseResults() {
  ...
  if (nEigFound > 0) {
    ...
    for (PetscInt iEig = 0; iEig < nEigFound; iEig++) {
      ...
      // Report
      output << "\t" << formatEig(reEigBout, imEigBout) << "\t(" << abs(boutEig) << ")"
             << endl;
