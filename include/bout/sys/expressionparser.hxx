...
public:
  ...
  virtual std::shared_ptr<FieldGenerator>
...
private:
  std::map<std::string, std::shared_ptr<FieldGenerator>>
      gen; ///< Generators, addressed by name
  std::map<char, std::pair<std::shared_ptr<FieldGenerator>, int>>
  ...
  struct LexInfo {
    ...
    signed char
        curtok;    ///< Current token. -1 for number, -2 for string, 0 for "end of input"
  ...
  std::shared_ptr<FieldGenerator> parseBinOpRHS(LexInfo &lex, int prec,
...
public:
  FieldBinary(std::shared_ptr<FieldGenerator> l, std::shared_ptr<FieldGenerator> r,
              char o)
      : lhs(l), rhs(r), op(o) {}
...
public:
  ...
  clone(const std::list<std::shared_ptr<FieldGenerator>> UNUSED(args)) {
    return std::shared_ptr<FieldGenerator>(
        std::shared_ptr<FieldGenerator>(new FieldValue(value)));
  }
