...
public:
   ...
   */
  inline BoutReal &operator[](const DataIterator &d) { return operator()(d.x, d.z); }
  inline const BoutReal &operator[](const DataIterator &d) const {
    return operator()(d.x, d.z);
  }
  BoutReal &operator[](const Indices &i) { return operator()(i.x, i.z); }
  ...
  BoutReal &operator()(int jx, int jz) {
    ...
    if ((jx < 0) || (jx >= nx) || (jz < 0) || (jz >= nz))
      throw BoutException("FieldPerp: (%d, %d) operator out of bounds (%d, %d)", jx, jz,
                          nx, nz);
#endif
...
inline const FieldPerp operator+(BoutReal lhs, const FieldPerp &rhs) { return rhs + lhs; }
