...
bool GridFile::get(Mesh *UNUSED(m), int &ival, const string &name) {
  ...
  if (success) {
    output_info << "\tOption " << name << " = " << ival << " (" << filename << ")"
                << endl;
  }
...
bool GridFile::get(Mesh *UNUSED(m), BoutReal &rval, const string &name) {
  ...
  if (success) {
    output_info << "\tOption " << name << " = " << rval << " (" << filename << ")"
                << endl;
  }
...
bool GridFile::get(Mesh *m, Field2D &var, const string &name, BoutReal def) {
  ...
  if (!file->is_valid()) {
    throw BoutException("Could not read '%s' from file: File cannot be read",
                        name.c_str());
  }
  ...
  if (field_dimensions[0] == m->GlobalNx) { /// including ghostpoints
    ...
    throw BoutException(
        "Could not read '%s' from file: x-dimension = %i do neither match nx = %i"
        "nor nx-2*mxg = %i ",
        name.c_str(), field_dimensions[0], m->GlobalNx, m->GlobalNx - 2 * mxg);
  }
  ...
  if (field_dimensions[1] == m->GlobalNy) { /// including ghostpoints
    ...
    throw BoutException(
        "Could not read '%s' from file: y-dimension = %i do neither match ny = %i"
        "nor ny-2*myg = %i ",
        name.c_str(), field_dimensions[1], m->GlobalNy, m->GlobalNy - 2 * myg);
  }
...
bool GridFile::get(Mesh *UNUSED(m), vector<int> &var, const string &name, int len,
                   int offset, GridDataSource::Direction UNUSED(dir)) {
...
bool GridFile::get(Mesh *UNUSED(m), vector<BoutReal> &var, const string &name, int len,
                   int offset, GridDataSource::Direction UNUSED(dir)) {
 ...
 */
bool GridFile::readgrid_3dvar_fft(Mesh *m, const string &name, int yread, int ydest,
                                  int ysize, int xge, int xlt, Field3D &var) {
 ...
 * any processing
 */
bool GridFile::readgrid_3dvar_real(Mesh *m, const string &name, int yread, int ydest,
                                   int ysize, int xge, int xlt, Field3D &var) {
  /// Check the arguments make sense
  if ((yread < 0) || (ydest < 0) || (ysize < 0) || (xge < 0) || (xlt < 0)) {
    return false;
