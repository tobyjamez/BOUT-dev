...
const Field2D Grad_par(const Field2D &var, DIFF_METHOD method,
                       CELL_LOC outloc = CELL_DEFAULT);
const Field3D Grad_par(const Field3D &var, CELL_LOC outloc = CELL_DEFAULT,
                       DIFF_METHOD method = DIFF_DEFAULT);
const Field3D Grad_par(const Field3D &var, DIFF_METHOD method,
                       CELL_LOC outloc = CELL_DEFAULT);
 ...
 *
 */
const Field3D Div_par(const Field3D &f, DIFF_METHOD method,
                      CELL_LOC outloc = CELL_DEFAULT);
 ...
 * @param[in] outloc  The cell location where the result is defined. By default the same
 * as A.
 */
const Field3D b0xGrad_dot_Grad(const Field3D &phi, const Field2D &A,
                               CELL_LOC outloc = CELL_DEFAULT);
const Field3D b0xGrad_dot_Grad(const Field2D &phi, const Field3D &A);
const Field3D b0xGrad_dot_Grad(const Field3D &phi, const Field3D &A,
                               CELL_LOC outloc = CELL_DEFAULT);
 ...
 * @param[in] outloc   The cell location where the result is defined. Default is the same
 * as g
