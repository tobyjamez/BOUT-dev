...
int PetscSolver::init(int NOUT, BoutReal TIMESTEP) {
  ...
  ierr = TSSetRHSFunction(ts, solver_f, this);
  CHKERRQ(ierr);
  ...
  // Sets the general-purpose update function called after every time step -- Copy
  // variables?
  ...
  CHKERRQ(ierr);
  ierr = TSSetUp(ts);
  CHKERRQ(ierr); // enable correct query of pctype below

  ierr = PetscTypeCompare((PetscObject)ts, TSSUNDIALS, &sundialstype);
  CHKERRQ(ierr);
  if (sundialstype) {
    ierr = TSSundialsGetPC(ts, &pc);
    CHKERRQ(ierr); // Sundials does not use SNES!
  } else {
  ...
  if (J_load) {
    ...
    CHKERRQ(ierr);
    ierr = MatSeqSBAIJSetPreallocation(J, dof, prealloc, PETSC_NULL);
    CHKERRQ(ierr);
    ierr =
        MatMPISBAIJSetPreallocation(J, dof, prealloc, PETSC_NULL, prealloc, PETSC_NULL);
    CHKERRQ(ierr);
    ...
    if (J_slowfd) { // create Jacobian matrix by slow fd
      ...
      CHKERRQ(ierr);
      ierr = TSDefaultComputeJacobian(ts, simtime, u, &J, &J, &J_structure, this);
      CHKERRQ(ierr);
      ...
      ierr = ISLocalToGlobalMappingCreate(comm, n, ltog_array, &ltog);
      CHKERRQ(ierr);
      ...
      CHKERRQ(ierr);
      ierr = MatSetLocalToGlobalMapping(J, ltog);
      CHKERRQ(ierr);
      ierr = MatSetLocalToGlobalMappingBlock(J, ltogb);
      CHKERRQ(ierr);
  ...
  if (J_write) { /* write J into a binary file for viewing its data structure */
    ...
    CHKERRQ(ierr);
    ierr = PetscViewerBinaryOpen(comm, "data_petsc/J.dat", FILE_MODE_WRITE, &viewer);
    CHKERRQ(ierr);
  ...
  ierr = MatGetColoring(J, MATCOLORING_SL, &iscoloring);
  CHKERRQ(ierr);
  ...
  ierr = TSDefaultComputeJacobianColor(ts, 0.0, u_petsc, &J, &J, &J_structure,
                                       matfdcoloring);
  CHKERRQ(ierr);
  ierr = PetscSynchronizedPrintf(data.comm,
                                 "[%d] TSDefaultComputeJacobianColor is done\n", rank);
  ierr = PetscSynchronizedFlush(data.comm);
  CHKERRQ(ierr);
#endif
...
PetscErrorCode PreStep(TS ts) {
  ...
  CHKERRQ(ierr);
  ierr = TSGetApplicationContext(ts, (void **)&s);
  CHKERRQ(ierr);
...
PetscErrorCode PostStep(TS ts) { PetscFunctionReturn(0); }
