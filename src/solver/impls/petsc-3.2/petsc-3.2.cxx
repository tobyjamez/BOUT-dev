...
int PetscSolver::init(int NOUT, BoutReal TIMESTEP) {
  ...
  ierr = TSSetInitialTimeStep(ts, simtime, initial_tstep);
  CHKERRQ(ierr);
  next_output = simtime;
  ...
  ierr = PetscTypeCompare((PetscObject)pc, PCNONE, &pcnone);
  CHKERRQ(ierr);
  if (pcnone)
    return (0);
  ...
  if (J_load) {
    ...
    /* number of z points (need to subtract one because of historical reasons that MZ has
     * an extra point) */
    PetscInt nz = mesh->LocalNz;

    /* number of degrees (variables) at each grid point */
    if (n2Dvars() != 0)
      throw BoutException("PETSc 3.2 solver can't handle 2D variables yet. Sorry\n");
    ...
    if (J_slowfd) { // create Jacobian matrix by slow fd
      ...
      ierr = TSSetRHSJacobian(ts, J, J, TSDefaultComputeJacobian, PETSC_NULL);
      CHKERRQ(ierr); // cannot set TSSetRHSJacobian(ts,Jmf,J,...)?
