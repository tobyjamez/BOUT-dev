...
void FieldData::addBndryFunction(FuncPtr userfunc, BndryLoc location) {
  ...
  addBndryGenerator(std::shared_ptr<FieldGenerator>(new FieldFunction(userfunc)),
                    location);
}

void FieldData::addBndryGenerator(std::shared_ptr<FieldGenerator> gen,
                                  BndryLoc location) {
...
std::shared_ptr<FieldGenerator> FieldData::getBndryGenerator(BndryLoc location) {
  std::map<BndryLoc, std::shared_ptr<FieldGenerator>>::iterator it =
      bndry_generator.find(location);
