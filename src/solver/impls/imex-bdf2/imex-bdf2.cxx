...
int IMEXBDF2::init(int nout, BoutReal tstep) {
  ...
  if ((ninternal == 0) || (out_timestep / ninternal > timestep))
    ++ninternal;
  if (ninternal > mxstep) {
    throw BoutException("Error: Number of internal timesteps (%i) exceeds mxstep (%i)",
                        ninternal, mxstep);
  };
  ...
  if (maxOrder > MAX_SUPPORTED_ORDER) {
    throw BoutException("Requested maxOrder greater than MAX_SUPPORTED_ORDER (%i)",
                        MAX_SUPPORTED_ORDER);
  }
...
void IMEXBDF2::constructSNES(SNES *snesIn) {
  ...
  if (matrix_free) {
    ...
    if (use_coloring) {
      ...
      // MatFDColoringSetFunction(fdcoloring,(PetscErrorCode
      // (*)(void))FormFunctionForDifferencing,this);
      ...
      */
      ...
      while (running) {
        ...
        if (dtNext <
          ...
          throw BoutException("Aborting: Timestep (%f) tried to go below minimum allowed",
                              dtNext);
        ...
        if (checkingErr) {
          ...
          if (adaptCounter > mxstepAdapt) {
            throw BoutException(
                "Aborting: Maximum number of adapative iterations (%i) exceeded",
                mxstepAdapt);
          }
      ...
      if (counter > mxstep) {
        throw BoutException(
            "Aborting: Maximum number of internal iterations (%i) exceeded", mxstep);
      };
    }
...
void IMEXBDF2::calculateCoeffs(int order) {
  ...
  case 4: {
    ...
    uFac[3] = -((1 + omega3) / (1 + omega1)) * (A2 / A1) * pow(omega1, 4) *
              pow(omega2, 3) * pow(omega3, 2) / A3;
  ...
  default: { throw BoutException("Invalid order supplied in IMEXBDF2::calculateCoeffs"); }
  };
 ...
 *
 */
template <class Op> void IMEXBDF2::loopVars(BoutReal *u) {
