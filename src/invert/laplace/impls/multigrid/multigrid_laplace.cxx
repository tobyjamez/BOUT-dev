...
LaplaceMultigrid::LaplaceMultigrid(Options *opt)
    : Laplacian(opt), A(0.0), C1(1.0), C2(1.0), D(1.0) {
  ...
  int implemented_boundary_flags =
      INVERT_AC_GRAD + INVERT_SET + INVERT_DC_GRAD; // INVERT_DC_GRAD does not actually do
                                                    // anything, but harmless to set while
                                                    // comparing to Fourier solver with
  ...
  if (mglevel > 1) {
    ...
    for (int n = mglevel; n > 1; n--) {
      if (nn % 2 != 0) {
        output << "Size of global x-domain is not a multiple of 2^" << mglevel - 1
               << " mglevel is changed to " << mglevel - n + 1 << endl;
        mglevel = mglevel - n + 1;
        break;
      }
      nn = nn / 2;
    ...
    for (int n = mglevel; n > 1; n--) {
      if (nn % 2 != 0) {
        output << "Size of global z-domain is not a multiple of 2^ " << mglevel - 1
               << " mglevel is changed to " << mglevel - n + 1 << endl;
        mglevel = mglevel - n + 1;
        break;
      }
  ...
  if (mglevel > 1) {
    ...
    for (int n = aclevel; n > 1; n--) {
      if (nn % 2 != 0) {
        output << "Size of local x-domain is not a multiple of 2^" << aclevel
               << " aclevel is changed to " << aclevel - n + 1 << endl;
        aclevel = aclevel - n + 1;
        break;
      }
      nn = nn / 2;
    ...
    for (int n = aclevel; n > 1; n--) {
      if (nn % 2 != 0) {
        output << "Size of local z-domain is not a multiple of 2^ " << aclevel
               << " aclevel is changed to " << aclevel - n << endl;
        aclevel = aclevel - n + 1;
        break;
      }
      nn = nn / 2;
  ...
  int rcheck = 0;
  if ((pcheck == 1) && (mgcount == 0))
    rcheck = 1;
  kMG = new Multigrid1DP(aclevel, Nx_local, Nz_local, Nx_global, adlevel, mgmpi, commX,
                         rcheck);
  kMG->mgplag = mgplag;
  kMG->mgsm = mgsm;
  kMG->cftype = cftype;
...
for (int k = 1; k < lzz + 1; k++) {
  int k2 = k - 1;
  x[k] = -x0(mesh->xstart - 1, k2) * sqrt(coords->g_11(mesh->xstart, yindex)) *
...
for (int k = 1; k < lzz + 1; k++) {
  int k2 = k - 1;
  x[(lxx + 1) * lz2 + k] = x0(mesh->xend + 1, k2) *
...
t1 = MPI_Wtime();
if ((mgcount == 300) && (tcheck != pcheck))
  tcheck = pcheck;
soltime += t1 - t0;
if (mgcount % 300 == 0) {
  output << "Accumulated execution time at " << mgcount << " Sol " << soltime << " ( "
         << settime << " )" << endl;
}
...
for (int i = 1; i < llx + 1; i++) {
  for (int k = 1; k < llz + 1; k++) {
    ...
    BoutReal ddz_C = (C2(i2, yindex, k2p) - C2(i2, yindex, k2m)) / 2. / coords->dz /
                     C1(i2, yindex, k2);
    ...
    BoutReal dxd =
        (D(i2, yindex, k2) * coords->G1(i2, yindex) + coords->g11(i2, yindex) * ddx_C +
         coords->g13(i2, yindex) * ddz_C // (could assume zero, at least initially, if
                                         // easier; then check this is true in
                                         // constructor)
         ) /
    ...
    BoutReal dzd =
        (D(i2, yindex, k2) * coords->G3(i2, yindex) + coords->g33(i2, yindex) * ddz_C +
         coords->g13(i2, yindex) * ddx_C // (could assume zero, at least initially, if
                                         // easier; then check this is true in
                                         // constructor)
         ) /
    ...
    mat[ic * 9 + 4] =
        A(i2, yindex, k2) - 2. * (ddx + ddz); // coefficient of no-derivative component
