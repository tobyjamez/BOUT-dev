...
// Parameters
BoutReal rho_s, Omega_i, c_s,
    n0; // Bohm gyro radius, Ion cyclotron frequency, Bohm sound speed
...
int physics_init(bool restarting) {
  ...
  if (boussinesq) {
    phiSolver =
        LaplaceXZ::create(mesh,
                          Options::getRoot()->getSection(
                              "phiBoussinesq")); // BOUT.inp section "phiBoussinesq"
    // Set the coefficients once here
    phiSolver->setCoefs(Field2D(1.0), Field2D(0.0));
  } else {
    phiSolver = LaplaceXZ::create(
        mesh,
        Options::getRoot()->getSection("phiSolver")); // BOUT.inp section "phiSolver"
