...
int physics_run(BoutReal time) {
  ...
  // Vorticity equation
  ddt(Vort) =
      B0 * B0 * Grad_parP_LtoC(jpar / B0) - B0 * Kappa(P) // Total perturbed pressure
      ;
  ...
  if (!(estatic && ZeroElMass)) {
    ...
    ddt(Ajpar) = Grad_parP_CtoL(Pe - phi) // Electron pressure only
  ...
  // Parallel velocity
  ddt(Vpar) = -Grad_parP_CtoL(P) // Total pressure
  ...
  ddt(Pe) = -bracket(phi, Pet, bm) +
            Pet * (Kappa(phi - Pe) + B0 * Grad_parP_LtoC((jpar - Vpar) / B0));
