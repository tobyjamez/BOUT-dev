  ...
  for (int x = 1; x <= mesh->LocalNx - 2; x++) {
    for (int y = mesh->ystart; y <= mesh->yend; y++) {
      ...
      for (int z = 0; z < ncz; z++) {
        ...
        // bz = DDX(f)
        BoutReal bz =
            (apar(x + 1, y, z) - apar(x - 1, y, z)) /
            (0.5 * metric->dx(x - 1, y) + metric->dx(x, y) + 0.5 * metric->dx(x + 1, y));
...
const Field3D Vpar_Grad_par(const Field3D &v, const Field3D &f, DIFF_METHOD method,
                            CELL_LOC outloc) {
  return mesh->coordinates()->Vpar_Grad_par(v, f, outloc, method);
}
...
const Field3D Div_par_flux(const Field3D &v, const Field3D &f, CELL_LOC outloc,
                           DIFF_METHOD method) {
  Coordinates *metric = mesh->coordinates();
  return -metric->Bxy * FDDY(v, f / metric->Bxy, outloc, method) / sqrt(metric->g_22);
}

const Field3D Div_par_flux(const Field3D &v, const Field3D &f, DIFF_METHOD method,
                           CELL_LOC outloc) {
...
const Field3D Grad_par_CtoL(const Field3D &var) {
  ...
  if (var.hasYupYdown()) {
    ...
    for (int jx = 0; jx < mesh->LocalNx; jx++) {
      for (int jy = 1; jy < mesh->LocalNy; jy++) {
        for (int jz = 0; jz < mesh->LocalNz; jz++) {
          result(jx, jy, jz) = 2. * (var(jx, jy, jz) - var.ydown()(jx, jy - 1, jz)) /
    ...
    for (int jx = 0; jx < mesh->LocalNx; jx++) {
      for (int jy = 1; jy < mesh->LocalNy; jy++) {
        for (int jz = 0; jz < mesh->LocalNz; jz++) {
          result(jx, jy, jz) = 2. * (var_fa(jx, jy, jz) - var_fa(jx, jy - 1, jz)) /
                               (metric->dy(jx, jy) * sqrt(metric->g_22(jx, jy)) +
                                metric->dy(jx, jy - 1) * sqrt(metric->g_22(jx, jy - 1)));
        }
      }
 ...
 */
CELL_LOC bracket_location(const CELL_LOC &f_loc, const CELL_LOC &g_loc,
                          const CELL_LOC &outloc) {
  ...
  if (outloc == CELL_DEFAULT) {
    ...
    if (f_loc != g_loc) {
      throw BoutException(
          "Bracket currently requires both fields to have the same cell location");
    } else {
  ...
  if (f_loc != g_loc || f_loc != outloc) {
    throw BoutException("Bracket currently requires the location of both fields and the "
                        "output locaton to be the same");
  }
...
const Field3D bracket(const Field3D &f, const Field3D &g, BRACKET_METHOD method,
  ...
  case BRACKET_CTU: {
    ...
    for (int y = mesh->ystart; y <= mesh->yend; y++) {
      ...
      // Simplest form: use cell-centered velocities (no divergence included so not flux
      // conservative)
