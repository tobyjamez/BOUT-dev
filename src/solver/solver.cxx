...
void Solver::add(Field2D &v, const char *name) {
  ...
  ///       from modifying the initial perturbation (e.g. to prevent unphysical
  ///       situations)
  ...
  Options::getRoot()->getSection(name)->get("evolve_bndry", d.evolve_bndry,
                                            d.evolve_bndry);
...
void Solver::add(Field3D &v, const char *name) {
  ...
  if (mms_initialise) {
    ...
    v = fact->create3D("solution", Options::getRoot()->getSection(name), mesh,
                       v.getLocation());
...
void Solver::addMonitor(Monitor *mon, MonitorPosition pos) {
  if (mon->timestep > 0) { // not default
    ...
    if (!isMultiple(timestep, mon->timestep))
      throw BoutException("Couldn't add Monitor: %g is not a multiple of %g!", timestep,
                          mon->timestep);
...
void Solver::removeMonitor(Monitor *f) { monitors.remove(f); }
...
int Solver::getLocalN() {
  ...
  int local_N = (mesh->xend - mesh->xstart + 1) * (mesh->yend - mesh->ystart + 1) *
                (n2d + ncz * n3d); // NOTE: Not including extra toroidal point
...
void Solver::set_id(BoutReal *udata) { loop_vars(udata, SET_ID); }
...
void Solver::add_mms_sources(BoutReal t) {
  ...
  for (const auto &f : f2d) {
    *f.F_var += fact->create2D("source", Options::getRoot()->getSection(f.name), mesh,
                               (f.var)->getLocation(), t);
  }

  for (const auto &f : f3d) {
    *f.F_var += fact->create3D("source", Options::getRoot()->getSection(f.name), mesh,
                               (f.var)->getLocation(), t);
  }
}
