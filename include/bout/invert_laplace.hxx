...
// or in input option "flags"
  ...
  const int INVERT_IN_SYM  =  1024; // Use symmetry to enforce either zero-value or
zero-gradient
...
public:
  ...
  virtual const FieldPerp solve(const FieldPerp &b, const FieldPerp &UNUSED(x0)) {
    return solve(b);
  }
...
int invert_laplace(const FieldPerp &b, FieldPerp &x, int flags, const Field2D *a,
                   const Field2D *c = NULL, const Field2D *d = NULL);
...
const Field3D invert_laplace(const Field3D &b, int flags, const Field2D *a = NULL,
                             const Field2D *c = NULL, const Field2D *d = NULL);
