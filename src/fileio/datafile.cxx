...
Datafile::Datafile(Options *opt)
    : parallel(false), flush(true), guards(true), floats(false), openclose(true),
      enabled(true), shiftOutput(false), shiftInput(false), flushFrequencyCounter(0),
      flushFrequency(1), file(nullptr) {
...
Datafile::Datafile(Datafile &&other)
    : parallel(other.parallel), flush(other.flush), guards(other.guards),
      ...
      flushFrequency(other.flushFrequency), file(other.file.release()),
      int_arr(other.int_arr), BoutReal_arr(other.BoutReal_arr), f2d_arr(other.f2d_arr),
...
Datafile::Datafile(const Datafile &other)
    : parallel(other.parallel), flush(other.flush), guards(other.guards),
      ...
      Lz(other.Lz), enabled(other.enabled), shiftOutput(other.shiftOutput),
      shiftInput(other.shiftInput), flushFrequencyCounter(other.flushFrequencyCounter),
      flushFrequency(other.flushFrequency), file(nullptr), int_arr(other.int_arr),
      BoutReal_arr(other.BoutReal_arr), f2d_arr(other.f2d_arr), f3d_arr(other.f3d_arr),
...
bool Datafile::openr(const char *format, ...) {
  ...
  // If parallel do not want to write ghost points, and it is easier then to ignore the
  // boundary guard cells as well
  if (parallel) {
...
bool Datafile::read_f3d(const string &name, Field3D *f, bool save_repeat) {
  ...
  if (save_repeat) {
    if (!file->read_rec(&((*f)(0, 0, 0)), name, mesh->LocalNx, mesh->LocalNy,
                        mesh->LocalNz)) {
      if (init_missing) {
    ...
    if (!file->read(&((*f)(0, 0, 0)), name, mesh->LocalNx, mesh->LocalNy,
                    mesh->LocalNz)) {
      if (init_missing) {
        output_warn.write("\tWARNING: Could not read 3D field %s. Setting to zero\n",
                          name.c_str());
...
bool Datafile::write_f2d(const string &name, Field2D *f, bool save_repeat) {
  if (!f->isAllocated()) {
    throw BoutException("Datafile::write_f2d: Field2D '%s' is not allocated!",
                        name.c_str());
  }
  if (save_repeat) {
...
bool Datafile::write_f3d(const string &name, Field3D *f, bool save_repeat) {
  if (!f->isAllocated()) {
    throw BoutException("Datafile::write_f3d: Field3D '%s' is not allocated!",
                        name.c_str());
  }
  ...
  if (save_repeat) {
    return file->write_rec(&(f_out(0, 0, 0)), name, mesh->LocalNx, mesh->LocalNy,
                           mesh->LocalNz);
  } else {
    return file->write(&(f_out(0, 0, 0)), name, mesh->LocalNx, mesh->LocalNy,
                       mesh->LocalNz);
  }
}
