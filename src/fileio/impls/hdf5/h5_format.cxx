...
H5Format::H5Format(bool parallel_in) {
  ...
  fname = NULL;
  dataFile = -1;
  chunk_length = 10; // could change this to try to optimize IO performance (i.e. allocate
                     // new chunks of disk space less often)
...
#ifdef PHDF5
  if (parallel)
    if (H5Pset_dxpl_mpio(dataSet_plist, H5FD_MPIO_INDEPENDENT) <
        0) // Default, independent writes
           //     if (H5Pset_dxpl_mpio(dataSet_plist, H5FD_MPIO_COLLECTIVE) < 0) //
           //     Alternative, collective writes
      throw BoutException("Failed to set dataSet_plist");
#endif

  if (H5Eset_auto(H5E_DEFAULT, NULL, NULL) < 0) // Disable automatic printing of error
                                                // messages so that we can catch errors
                                                // without printing error messages to
                                                // stdout
...
const vector<int> H5Format::getSize(const string &var) { return getSize(var.c_str()); }
...
bool H5Format::setLocalOrigin(int x, int y, int z, int offset_x, int offset_y,
                              int offset_z) {
...
bool H5Format::read(void *data, hid_t hdf5_type, const char *name, int lx, int ly,
                    int lz) {
  ...
  init_size_local[2] = offset_local[2] + counts[2]; // Want to be able to use without
                                                    // needing mesh to be initialised;
                                                    // makes hyperslab selection redundant
...
bool H5Format::write(void *data, hid_t mem_hdf5_type, hid_t write_hdf5_type,
                     const char *name, int lx, int ly, int lz) {
  ...
  if (dataSet < 0) {
    ...
    dataSet = H5Dcreate(dataFile, name, write_hdf5_type, init_space, H5P_DEFAULT,
    ...
    hid_t myatt_in = H5Acreate(dataSet, "type", variable_length_string_type,
                               attribute_dataspace, H5P_DEFAULT, H5P_DEFAULT);
    if (myatt_in < 0)
...
bool H5Format::write_rec(void *data, hid_t mem_hdf5_type, hid_t write_hdf5_type,
                         const char *name, int lx, int ly, int lz) {
  if (!is_valid())
    return false;
  ...
  if (dataSet >= 0) { // >=0 means file exists, so open. Else create it.
    ...
    max_dims[1] = init_size[1];
    max_dims[2] = init_size[2];
    max_dims[3] = init_size[3];
    chunk_dims[0] = chunk_length;
    chunk_dims[1] = init_size[1];
    chunk_dims[2] = init_size[2];
    chunk_dims[3] = init_size[3];
