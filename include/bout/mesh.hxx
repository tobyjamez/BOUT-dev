...
public:
   ...
   */
  template <typename... Ts> void communicate(Ts &... ts) {
    FieldGroup g(ts...);
    communicate(g);
  }

  template <typename... Ts> void communicateXZ(Ts &... ts) {
    FieldGroup g(ts...);
    communicateXZ(g);
  }
   ...
   * to send(FieldGroup&).
   */
  template <typename... Ts> comm_handle send(Ts &... ts) {
    FieldGroup g(ts...);
    return send(g);
  }
  ...
  virtual MPI_Request sendToProc(int xproc, int yproc, BoutReal *buffer, int size,
                                 int tag) = 0;
  ...
  virtual comm_handle receiveFromProc(int xproc, int yproc, BoutReal *buffer, int size,
                                      int tag) = 0;
  ...
  // Y communications
  virtual bool firstY()
      const = 0; ///< Is this processor first in Y? i.e. is there a boundary at lower Y?
  virtual bool lastY()
      const = 0; ///< Is this processor last in Y? i.e. is there a boundary at upper Y?
  virtual bool firstY(int xpos)
      const = 0; ///< Is this processor first in Y? i.e. is there a boundary at lower Y?
  ...
  indexDDX(const Field3D &f, CELL_LOC outloc,
           DIFF_METHOD method); ///< First derivative in X direction, in index space
  const Field2D
  indexDDX(const Field2D &f); ///< First derivative in X direction, in index space

  const Field3D
  indexDDY(const Field3D &f, CELL_LOC outloc,
           DIFF_METHOD method); ///< First derivative in Y direction in index space
  const Field2D
  indexDDY(const Field2D &f); ///< ///< First derivative in Y direction in index space

  const Field3D
  indexDDZ(const Field3D &f, CELL_LOC outloc, DIFF_METHOD method,
           bool inc_xbndry); ///< First derivative in Z direction in index space
  const Field2D
  indexDDZ(const Field2D &f); ///< First derivative in Z direction in index space
  ...
  const Field2D
  indexD2DX2(const Field2D &f); ///< Second derivative in X direction in index space
  ...
  const Field2D
  indexD2DY2(const Field2D &f); ///< Second derivative in Y direction in index space
  ...
  const Field3D
  indexD4DX4(const Field3D &f); ///< Fourth derivative in X direction in index space
  const Field2D
  indexD4DX4(const Field2D &f); ///< Fourth derivative in X direction in index space
  const Field3D
  indexD4DY4(const Field3D &f); ///< Fourth derivative in Y direction in index space
  const Field2D
  indexD4DY4(const Field2D &f); ///< Fourth derivative in Y direction in index space
  const Field3D
  indexD4DZ4(const Field3D &f); ///< Fourth derivative in Z direction in index space
  ...
  typedef BoutReal (*deriv_func)(
      stencil &); ///< Derivative functions of a single field stencil
  typedef BoutReal (*upwind_func)(
      BoutReal,
  ...
  bool canToFromFieldAligned() { return getParallelTransform().canToFromFieldAligned(); }
   ...
   */
  void setParallelTransform(PTptr pt) { transform = std::move(pt); }
  /*!
  ...
  Region<> &getRegion(const std::string &region_name) { return getRegion3D(region_name); }
