...
Sources::~Sources() { delete position; }
...
void Sources::initialise() {
  ...
  lower_source_yindex = (mesh->GlobalNy + 1) / 2 - 3 -
                        int(source_length / 2. / sqrt(coord->g_22(2, 2)) /
                            coord->dy(2, 2)); // GlobalNy+1 in case GlobalNy is odd for
                                              // some strange reason, then the source is
                                              // still symmetrical and vanishes if
                                              // source_length=0
  upper_source_yindex =
      mesh->GlobalNy / 2 - 3 +
      int(source_length / 2. / sqrt(coord->g_22(2, 2)) / coord->dy(2, 2));
}
...
Field3D Sources::particle_source(BoutReal &t) {
  if (t > on_time && t < off_time) {
    if (!particle_during_transient) {
      ...
      do {
        if (mesh->YGLOBAL(position->jy) >= lower_source_yindex &&
            mesh->YGLOBAL(position->jy) <= upper_source_yindex) {
          particle_source_field[*position] =
              (particle_amplitude + particle_transient_amplitude) *
              sin(PI * (1. * (mesh->YGLOBAL(position->jy) - lower_source_yindex)) /
                  (1. * (upper_source_yindex - lower_source_yindex)));
          //           particle_source_field[*position] = (particle_amplitude +
          //           particle_transient_amplitude*PI/2*sin(PI*(t-on_time)/(off_time-on_time)))
          //           *
          //           sin(PI*(1.*(mesh->YGLOBAL(position->jy)-lower_source_yindex))/(1.*(upper_source_yindex-lower_source_yindex)));
        } else {
  ...
  if (t > on_time && t < off_time) {
    if (!electron_heat_during_transient) {
      ...
      do {
        if (mesh->YGLOBAL(position->jy) >= lower_source_yindex &&
            mesh->YGLOBAL(position->jy) <= upper_source_yindex) {
          electron_heat_source_field[*position] =
              (electron_heat_amplitude + electron_heat_transient_amplitude) *
              sin(PI * (1. * (mesh->YGLOBAL(position->jy) - lower_source_yindex)) /
                  (1. * (upper_source_yindex - lower_source_yindex)));
          //           electron_heat_source_field[*position] = (electron_heat_amplitude +
          //           electron_heat_transient_amplitude*PI/2*sin(PI*(t-on_time)/(off_time-on_time)))
          //           *
          //           sin(PI*(1.*(mesh->YGLOBAL(position->jy)-lower_source_yindex))/(1.*(upper_source_yindex-lower_source_yindex)));
        } else {
  ...
  if (t > on_time && t < off_time) {
    if (!ion_heat_during_transient) {
      ...
      do {
        if (mesh->YGLOBAL(position->jy) >= lower_source_yindex &&
            mesh->YGLOBAL(position->jy) <= upper_source_yindex) {
          ion_heat_source_field[*position] =
              (ion_heat_amplitude + ion_heat_transient_amplitude) *
              sin(PI * (1. * (mesh->YGLOBAL(position->jy) - lower_source_yindex)) /
                  (1. * (upper_source_yindex - lower_source_yindex)));
          //         ion_heat_source_field[*position] = (ion_heat_amplitude +
          //         ion_heat_transient_amplitude*PI/2*sin(PI*(t-on_time)/(off_time-on_time)))
          //         *
          //         sin(PI*(1.*(mesh->YGLOBAL(position->jy)-lower_source_yindex))/(1.*(upper_source_yindex-lower_source_yindex)));
        } else {
