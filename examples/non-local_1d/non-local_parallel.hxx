...
public:
  ...
  ~NonLocalParallel();
  int moments_number;
  void initialise(BoutReal pass_electron_charge, BoutReal pass_electron_mass,
                  BoutReal pass_ion_mass, BoutReal pass_epsilon_0,
                  BoutReal pass_logLambda,
                  const bool pass_fluxes_location_is_ylow = false,
                  BoutReal pass_gamma_factor = 5. / 3.);
  ...
  void y_broadcast(void *input_buffer, const int &size,
                   const int &root_processor); // NB Assumes that the mesh is BoutMesh

  void rms_over_y(const Field3D &input_field, FieldPerp &output_field);
  void mean_over_y(const Field3D &input_field, FieldPerp &output_field,
                   int exclude_edgecells = 0);
...
private:
  ...
  Field3D
      increasing_dimensionless_length; // d/dl(increasing_dimensionless_length)=1/lambdaC
  Field3D decreasing_dimensionless_length;
  Field3D dimensionless_length_deltas_above; // change in dimensionless_length between jy
                                             // and jy+1, used in calculating integrals.
                                             // (More accurate than first adding up and
...
#ifdef DRIVE_GRADT
  Field3D gradT_driveterm; // drive from Maxwellian moments for heat flux calculation.
                           // g^(1,1)/T^1.5 as a function of l.
  // NB slightly bad notation: drive_term will INCLUDE grad(T) for cell-centred version
  // but EXCLUDE grad(T) (i.e. be only the cell-centred prefactor) for the ylow staggered
  // version, so that grad(T) can be interpolated separately
  ...
  void calculate_nonlocal_closures_cell_centre(const Field3D &n_electron,
