...
LaplaceMumps::LaplaceMumps(Options *opt)
    : Laplacian(opt), A(0.0), C1(1.0), C2(1.0), D(1.0), Ex(0.0), Ez(0.0), issetD(false),
      issetC(false), issetE(false) {
  ...
  implemented_boundary_flags = INVERT_AC_GRAD + INVERT_RHS;
  if (global_flags & ~implemented_flags) {
    if (global_flags & INVERT_4TH_ORDER)
      output << "For MUMPS based Laplacian inverter, use 'fourth_order=true' instead of "
                "setting INVERT_4TH_ORDER flag"
             << endl;
    throw BoutException("Attempted to set Laplacian inversion flag that is not "
                        "implemented in mumps_laplace.cxx");
  }
  if (inner_boundary_flags & ~implemented_boundary_flags) {
    throw BoutException("Attempted to set Laplacian inversion boundary condition flag "
                        "that is not implemented in mumps_laplace.cxx");
  }
  ...
  if (mesh->periodicX) {
    throw BoutException("LaplaceMumps does not work with periodicity in the x direction "
  ...
  if (mesh->firstX())
    localN += mesh->xstart *
              (mesh->LocalNz); // If on first processor add on width of boundary region
  if (mesh->lastX())
    localN += mesh->xstart *
              (mesh->LocalNz); // If on last processor add on width of boundary region
  ...
  mumps_struc.comm_fortran = (MUMPS_INT)MPI_Comm_c2f(
      mesh->getXcomm()); // MPI communicator for MUMPS, in fortran format
  ...
  mumps_struc.n = size;
  // nz is the total number of non-zero elements in the matrix, nz_loc is the number of
  ...
  if (inner_boundary_flags & INVERT_AC_GRAD) {
    if (fourth_order) {
      ...
      if (mesh->firstX())
        mumps_struc.nz_loc += 5 * mesh->xstart * (mesh->LocalNz);
    } else {
      mumps_struc.nz += 3 * meshz * (mesh->xstart);
      if (mesh->firstX())
        mumps_struc.nz_loc += 3 * mesh->xstart * (mesh->LocalNz);
    }
  } else {
    mumps_struc.nz += mesh->xstart * meshz;
    if (mesh->firstX())
      mumps_struc.nz_loc += mesh->xstart * (mesh->LocalNz);
  }
  if (outer_boundary_flags & INVERT_AC_GRAD) {
    if (fourth_order) {
      mumps_struc.nz += 5 * (mesh->LocalNx - mesh->xend - 1) * meshz;
      if (mesh->lastX())
        mumps_struc.nz_loc += 5 * (mesh->LocalNx - mesh->xend - 1) * (mesh->LocalNz);
    } else {
      mumps_struc.nz += 3 * (mesh->LocalNx - mesh->xend - 1) * meshz;
      if (mesh->lastX())
        mumps_struc.nz_loc += 3 * (mesh->LocalNx - mesh->xend - 1) * (mesh->LocalNz);
    }
  } else {
    mumps_struc.nz += (mesh->LocalNx - mesh->xend - 1) * meshz;
    if (mesh->lastX())
      mumps_struc.nz_loc += (mesh->LocalNx - mesh->xend - 1) * (mesh->LocalNz);
  }
  // // These would be needed if giving the matrix only on the host processor, or possibly
  // if providing the structure on the host processor for analysis
  ...
  mumps_struc.irn_loc = new MUMPS_INT[mumps_struc.nz_loc]; // list of GLOBAL row indices
                                                           // of local matrix entries
  mumps_struc.jcn_loc = new MUMPS_INT[mumps_struc.nz_loc]; // list of GLOBAL column
                                                           // indices of local matrix
                                                           // entries
  ...
  if (mesh->firstX()) {
    ...
    //     mumps_struc.rhs = new BoutReal[mumps_struc.lrhs*mumps_struc.nrhs]; // rhs, to
    //     be provided on the rank-0 processor only
  }
  //   if (mesh->firstX()) mumps_struc.sol_loc = *sol.getData(); // pointer to the array
  //   to put the solution in, starts at 0 on first processor
  //   else mumps_struc.sol_loc = *sol.getData() + mesh->xstart*meshz; // pointer to the
  //   array to put the solution in, starts at mesh->xstart
  //   mumps_struc.lsol_loc = localN; // size of the (local) solution array
  //   mumps_struc.isol_loc = new MUMPS_INT[localN]; // list of indices of the solution
  //   array (though this is all local points)
  ...
  mumps_struc.icntl[17] =
      3; // provide distributed matrix for both analysis and factorization
  // Don't use this: gives solution in some sructure convenient for the solver, not as
  // desired by BOUT++       mumps_struc.icntl[20] = 1; // leave the solution distributed
  // across the processors
  //   mumps_struc.icntl[22] = ; // Apparently should provide a value here significantly
  //   larger than infog[15] when running in parallel
  mumps_struc.icntl[27] = 2; // Force parallel analysis
  mumps_struc.icntl[28] = 0; // set to 1 to force pt-scotch and to 2 to force parmetis to
                             // be used for the ordering
  //   mumps_struc.cntl[0] = 0.01; // relative threshold for numerical pivoting. 0.0 is
  //   appropriate if matrix is diagonally dominant (there are no zero pivots). High
  //   values increase fill-in but lead to more accurate factorization. Default is 0.01
  ...
  //       localrhs_size_array[nxpe-1] =
  //       (mesh->LocalNx-mesh->xstart)*mesh->LocalNy*mesh->LocalNz;
  //     }
  ...
  // Set Arrays of matrix indices, using i (0<=i<nz_loc), and solution indices, using j
  // (0<=j<localN)
  int i = 0; // int j=0;
  if (mesh->firstX())
    for (int x = 0; x < mesh->xstart; x++)
      for (int z = 0; z < mesh->LocalNz; z++) {
        ...
        int z0 = z;
        if (inner_boundary_flags & INVERT_AC_GRAD) {
          mumps_struc.irn_loc[i] =
              x0 * meshz + z0 + 1; // Indices for fortran arrays that start at 1
        ...
        // 	mumps_struc.isol_loc[j] = x0*meshz + z0 + 1; // Indices for fortran arrays
        // that start at 1
        // 	j++;
      }
  ...
  if (i != mumps_struc.nz_loc)
    throw BoutException("LaplaceMumps: matrix index error");
...
//     if (include_yguards)
//       ye = mesh->LocalNy-1; // Contains upper boundary and we are solving in the guard
//       cells
//
//     ye -= extra_yguards_upper;
//   }
//
//   Field3D x = copy(b); // Force new memory allocation as we will mess around with x's
//   data via pointers (i.e. 'unsafely')
//
//   BoutReal* localrhs = **x.getData(); // Input the rhs in the solution field as
//   solution will be returned in place by MUMPS
...
// 	    rhs_slice[jx*meshz+jz] = rhs[jx*mesh->LocalNy*mesh->LocalNz + jy*mesh->LocalNz
// + jz];
//
//       solve(rhs_slice,jy);
//
...
// 	for(int jx=0; jx<meshx; jx++)
// 	  for (int jz=0; jz<meshz; jz++)
// 	     rhs[jx*mesh->LocalNy*mesh->LocalNz + jy*mesh->LocalNz + jz] =
// rhs_slice[jx*meshz+jz];
//     }
//     mumps_struc.job = MUMPS_JOB_BOTH;
//   }
//   else {
...
//   }
//
//   MPI_Scatterv(rhs,localrhs_size_array,rhs_positions,MPI_DOUBLE,localrhs,localrhssize,MPI_DOUBLE,0,mesh->getXcomm());
//   // Scatters solution from host back to localrhs (which points to x's data) on all
//   processors
//
// #if CHECK > 0
//   msg_stack.pop();
...
const FieldPerp LaplaceMumps::solve(const FieldPerp &b) {
  ...
  MPI_Gatherv(localrhs, localrhssize, MPI_DOUBLE, rhs, localrhs_size_array, rhs_positions,
              MPI_DOUBLE, 0, comm);

  solve(rhs, y);

  MPI_Scatterv(rhs, localrhs_size_array, rhs_positions, MPI_DOUBLE, localrhs,
               localrhssize, MPI_DOUBLE, 0, comm); // Scatters solution from host back to
                                                   // localrhs (which points to x's data)
                                                   // on all processors
...
void LaplaceMumps::solve(BoutReal *rhs, int y) {

  {
    ...
    // Set Matrix Elements corresponding to index lists created in constructor (x,z) loop
    // over rows
    ...
    if (mesh->firstX())
      for (int x = 0; x < mesh->xstart; x++)
        for (int z = 0; z < mesh->LocalNz; z++) {
          // Set values corresponding to nodes adjacent in x if Neumann Boundary
          // Conditions are required.
          if (inner_boundary_flags & INVERT_AC_GRAD)
            if (fourth_order) {
              // Fourth Order Accuracy on Boundary
              mumps_struc.a_loc[i] =
                  -25.0 / (12.0 * coord->dx[x][y]) / sqrt(coord->g_11[x][y]);
              i++;
              ...
              i++;
              mumps_struc.a_loc[i] =
                  4.0 / (3.0 * coord->dx[x][y]) / sqrt(coord->g_11[x][y]);
              i++;
              mumps_struc.a_loc[i] =
                  -1.0 / (4.0 * coord->dx[x][y]) / sqrt(coord->g_11[x][y]);
              i++;
            } else {
              // Second Order Accuracy on Boundary
              mumps_struc.a_loc[i] =
                  -3.0 / (2.0 * coord->dx[x][y]) / sqrt(coord->g_11[x][y]);
              i++;
              mumps_struc.a_loc[i] = 2.0 / coord->dx[x][y] / sqrt(coord->g_11[x][y]);
              i++;
              mumps_struc.a_loc[i] =
                  -1.0 / (2.0 * coord->dx[x][y]) / sqrt(coord->g_11[x][y]);
              i++;
            }
          else {
...
void LaplaceMumps::Coeffs(int x, int y, int z, BoutReal &coef1, BoutReal &coef2,
  ...
  if (nonuniform) {
    ...
    if ((x != 0) && (x != (mesh->LocalNx - 1))) {
      // coef4 += coord->g11[jx][jy]*0.25*( (1.0/dx[jx+1][jy]) - (1.0/dx[jx-1][jy])
      // )/dx[jx][jy]; // SHOULD BE THIS (?)
      // coef4 -= 0.5 * ( ( coord->dx[x+1][y] - coord->dx[x-1][y] ) / SQ ( coord->dx[x][y]
      // ) ) * coef1; // BOUT-06 term
      coef4 -= 0.5 *
               ((coord->dx[x + 1][y] - coord->dx[x - 1][y]) / pow(coord->dx[x][y], 2.0)) *
               coef1; // BOUT-06 term
    }
  }
  ...
  if (issetC) {
    //     if( (x > 0) && (x < (mesh->LocalNx-1)) ) //Valid if doing second order
    //     derivative, not if fourth: should only be called for xstart<=x<=xend anyway
    if ((x > 1) && (x < (mesh->LocalNx - 2))) {
      ...
      if (fourth_order) {
        ...
        ddx_C = (-C2[x + 2][y][z] + 8. * C2[x + 1][y][z] - 8. * C2[x - 1][y][z] +
                 C2[x - 2][y][z]) /
                (12. * coord->dx[x][y] * (C1[x][y][z]));
        ddz_C = (-C2[x][y][zpp] + 8. * C2[x][y][zp] - 8. * C2[x][y][zm] + C2[x][y][zmm]) /
                (12. * coord->dz * (C1[x][y][z]));
      } else {
        ddx_C =
            (C2[x + 1][y][z] - C2[x - 1][y][z]) / (2. * coord->dx[x][y] * (C1[x][y][z]));
  ...
  // setCoefEx()/setCoefEz() because both (in principle) are needed and we don't know how
  // to split them up here
  if (issetE) {
    coef4 += Ex[x][y][z];
    coef5 += Ez[x][y][z];
  }
