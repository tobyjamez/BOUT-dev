...
int PetscSolver::init(int NOUT, BoutReal TIMESTEP) {
  ...
  CHKERRQ(ierr);
  ierr = TSSetExactFinalTime(ts, PETSC_TRUE);
  CHKERRQ(ierr);
  ...
  if (J_load) {
    ...
    if (J_slowfd) { // create Jacobian matrix by slow fd
      ...
      ierr = SNESSetJacobian(snes, J, J, SNESDefaultComputeJacobian, PETSC_NULL);
      CHKERRQ(ierr);
  ...
  CHKERRQ(ierr);
  ierr = SNESSetJacobian(snes, J, J, SNESDefaultComputeJacobianColor, matfdcoloring);
  CHKERRQ(ierr);
