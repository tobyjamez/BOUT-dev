...
public:
  LaplaceMumps(Options *UNUSED(opt) = NULL) {
...
public:
  ...
  void setFlags(int f) {
    throw BoutException("May not change the value of flags during run in LaplaceMumps as "
                        "it might change the number of non-zero matrix elements: flags "
                        "may only be set in the options file.");
  }
...
private:
  ...
  //   int iteration_count; // Use this to count the number of iterations since last
  //   analysis
