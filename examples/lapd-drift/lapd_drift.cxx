...
protected:
  ...
  int init(bool restarting) {
    ...
    dump.add(coord->Bxy, "Bxy", 0);
    dump.add(my_ixseps, "ixseps", 0);
  ...
  int rhs(BoutReal t) {
    ...
    if (ZeroElMass) {
      ...
      jpar =
          ((Tet * Grad_par_LtoC(ni)) - (Nit * Grad_par_LtoC(phi))) / (fmei * 0.51 * nu);
    ...
    if (evolve_rho) {
      ...
      if (rho_rho1) {
        ddt(rho) += mu_i * Delp2(rho, -1.0); // Check second argument meaning in
                                             // difops.cpp
      }
    ...
    if (evolve_ajpar) {
      ...
      if (ajpar_phi1) {
        ddt(ajpar) +=
            (1. / fmei) *
            Grad_par_LtoC(phi); // Right-hand deriv with b.c. Necessary for sheath mode
      }
    ...
    if (evolve_te) {
      ...
      if (te_nu_te1) {
        ddt(te) -= 2. * fmei * nu_hat / sqrt(Te0) *
                   (ni - 1. / 2. * Ni0 / Te0 * te); // Explicitly linear
      }
