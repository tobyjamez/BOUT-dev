...
const Field3D DDZ(const Field3D &f, CELL_LOC outloc = CELL_DEFAULT,
                  DIFF_METHOD method = DIFF_DEFAULT, bool inc_xbndry = false);
 ...
 *
 */
const Vector3D DDZ(const Vector3D &v, CELL_LOC outloc = CELL_DEFAULT,
                   DIFF_METHOD method = DIFF_DEFAULT);
...
const Field2D FDDZ(const Field2D &v, const Field2D &f, CELL_LOC outloc,
                   DIFF_METHOD method = DIFF_DEFAULT);
const Field2D FDDZ(const Field2D &v, const Field2D &f, DIFF_METHOD method,
                   CELL_LOC outloc = CELL_DEFAULT);
