...
// const BoutReal Low_limit = 1.e-10;   // limit of the profile to prevent minus total
// value
...
int physics_init(bool restarting) {
  ...
  OPTION(options, n0_fake_prof, false); // use the hyperbolic profile of n0. If both
                                        // n0_fake_prof and T0_fake_prof are false, use
                                        // the profiles from grid file
  OPTION(options, n0_height,
  ...
  OPTION(options, laplace_alpha,
         1.0); // test parameter for the cross term of invert Lapalace
  ...
  LnLambda = 24.0 - log(pow(Zi * Nbar * density / 1.e6, 0.5) *
                        pow(Tebar, -1.0)); // xia: ln Lambda
...
int physics_run(BoutReal t) {
  ...
  if (nonlinear) {
    ...
    if (diffusion_par > 0.0) {
      ...
      nu_i = 4.80e-8 * (Zi * Zi * Zi * Zi / sqrt(AA)) * LnLambda *
             (N_tmp * Nbar * density / 1.e6) * pow(Ti_tmp * Tibar, -1.5); // nu_i in 1/S.
