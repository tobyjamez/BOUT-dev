...
protected:
  ...
  int rhs(BoutReal time) {
    ...
    // Energy equation:
    //   ddt(T) = bracket(phi,T,bm) + (2/3)*T*C(phi) - (7/3)*T*C(T) - (2/3)*(T*T/n)*C(n) +
    //   chi*Delp2(T) - sigma_T*T ;

    // Vorticity equation:
    //   ddt(vort) = bracket(phi, vort, bm) - C(n/**T*/) + mu*Delp2(vort) -
    //   sigma_vort*vort ;
    ddt(vort) =
        bracket(phi, vort, bm) - C(exp(N) /**T*/) + mu * Delp2(vort) - sigma_vort * vort;
    // ddt(vort) += phi/1.e4; // Sheath dissipation term to try to stop potential getting
    // too large...
