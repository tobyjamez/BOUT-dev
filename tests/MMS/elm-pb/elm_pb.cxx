...
int physics_run(BoutReal t) {
  ...
  // Get J from Psi
  // PsiExact = FieldFactory::get()->create3D("psi:solution", Options::getRoot(), mesh,
  // CELL_CENTRE, t);
  ...
  // Jpar = FieldFactory::get()->create3D("j:solution", Options::getRoot(), mesh,
  // CELL_CENTRE, t);
  // Jpar.applyBoundary(t);

  // phi = FieldFactory::get()->create3D("phi:solution", Options::getRoot(), mesh,
  // CELL_CENTRE, t);
  ...
  if (eHall) {
    ddt(Psi) +=
        0.25 * delta_i * (Grad_parP(B0 * P, CELL_CENTRE) / B0 +
