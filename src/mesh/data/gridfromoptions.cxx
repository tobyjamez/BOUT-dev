...
bool GridFromOptions::hasVar(const string &name) { return options->isSet(name); }
...
bool GridFromOptions::get(Mesh *m, vector<BoutReal> &var, const string &name, int len,
  ...
  case GridDataSource::Z: {
    for (int z = 0; z < len; z++) {
      var[z] = gen->generate(0.0, 0.0,
                             TWOPI * (static_cast<BoutReal>(z) + offset) /
                                 static_cast<BoutReal>(m->LocalNz),
                             0.0);
    }
    break;
  }
