...
protected:
  ...
  int rhs(BoutReal t) override { return physics_run(t); }
};

/// Global functions used by some legacy models
template <class T> void bout_solve(T &var, const char *name) { solver->add(var, name); }
...
bool bout_constrain(Field3D &var, Field3D &F_var, const char *name) {
  if (!solver->constraints())
    return false; // Doesn't support constraints
