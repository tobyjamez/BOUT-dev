 ...
 *
 */
class DataIterator : public std::iterator<std::bidirectional_iterator_tag, Indices> {
private:
/*!
...
public:
   ...
   */
  const DataIterator &operator*() const { return *this; }
...
#else  //_OPENMP
    return (x == xend && y == yend && z > zend) || (x == xend && y > yend) || x > xend ||
...
struct IndexRange {
  ...
  const DataIterator begin() const {
    return DataIterator(xstart, xend, ystart, yend, zstart, zend);
  }
  const DataIterator end() const {
    return DataIterator(xstart, xend, ystart, yend, zstart, zend, DI_GET_END);
  }
};
