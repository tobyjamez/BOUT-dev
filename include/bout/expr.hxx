...
public:
  ...
  const BoutReal &operator()(int x, int y, int z) const {
    return data[(x * mesh->LocalNy + y) * mesh->LocalNz + z];
  }
...
template <> struct exprTraits<float> { typedef Literal expr_type; };
...
template <class ExprT1, class ExprT2, class BinOp> class BinaryExpr {
public:
  BinaryExpr(const ExprT1 &e1, const ExprT2 &e2) : _expr1(e1), _expr2(e2) {}
...
template <typename ExprT1, typename ExprT2, class name> struct BinaryResult {
...
#define DEFINE_BINARY_OP(name, op)                                                       \
  struct name {                                                                          \
    template <typename T> static inline T apply(T a, T b) { return a op b; }             \
  };
...
struct Power {
  template <typename T> static inline T apply(T a, T b) { return pow(a, b); }
};

/// Define functions add, mul which use operator structs
#define DEFINE_OVERLOAD_FUNC(name, func)                                                 \
  template <typename ExprT1, typename ExprT2>                                            \
  typename BinaryResult<ExprT1, ExprT2, name>::type func(const ExprT1 &e1,               \
                                                         const ExprT2 &e2) {             \
...
template <typename Expr> const Field3D eval3D(Expr e) {
  Field3D result;
  result.allocate();
