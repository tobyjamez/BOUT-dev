...
public:
   ...
   * Copy constructor
   */
  Array(const Array &other) { ptr = other.ptr; }
   ...
   * Move constructor
   */
  Array(Array &&other) { ptr = std::move(other.ptr); }
...
// Make copy of the underlying data
  ...
  iterator begin() { return (ptr) ? ptr->data : nullptr; }

  iterator end() { return (ptr) ? ptr->data + ptr->len : nullptr; }

  // Const iterators
  const_iterator begin() const { return (ptr) ? ptr->data : nullptr; }

  const_iterator end() const { return (ptr) ? ptr->data + ptr->len : nullptr; }
#else
  iterator begin() { return (ptr) ? std::begin(*ptr) : nullptr; }

  iterator end() { return (ptr) ? std::end(*ptr) : nullptr; }

  // Const iterators -- should revisit with cbegin and cend in c++14 and greater
  const_iterator begin() const { return (ptr) ? std::begin(*ptr) : nullptr; }

  const_iterator end() const { return (ptr) ? std::end(*ptr) : nullptr; }
#endif
  //////////////////////////////////////////////////////////
  // Element access
   ...
   */
  void swap(Array<T> &other) { std::swap(ptr, other.ptr); }
 ...
 */
template <typename T> Array<T> &copy(const Array<T> &other) {
  Array<T> a(other);
  a.ensureUnique();
  return a;
}
 ...
 * Sizes of the arrays may differ.
 */
template <typename T> void swap(Array<T> &a, Array<T> &b) { a.swap(b); }
