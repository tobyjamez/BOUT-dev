...
inline BoutReal DDX_C4(const Field3D &f, const DataIterator &i) {
  return (8. * f[i.xp()] - 8. * f[i.xm()] + f[i.offset(-2, 0, 0)] -
          f[i.offset(2, 0, 0)]) /
         (12. * mesh->coordinates()->dx[i]);
}
...
inline BoutReal DDZ_C4(const Field3D &f, const DataIterator &i) {
  return (8. * f[i.zp()] - 8. * f[i.zm()] + f[i.offset(0, 0, -2)] -
          f[i.offset(0, 0, 2)]) /
         (12. * mesh->coordinates()->dz);
}
...
BoutReal bracket_arakawa(const Field3D &f, const Field3D &g, const DataIterator &i) {
  ...
  BoutReal Jpp = 0.25 * ((fzp - fzm) * (gxp - gxm) - (fxp - fxm) * (gzp - gzm));

  // J+x
  BoutReal Jpx = 0.25 * (gxp * (fpp - fpm) - gxm * (fmp - fmm) - gzp * (fpp - fmp) +
                         gzm * (fpm - fmm));
  // Jx+
  BoutReal Jxp =
      0.25 * (g[i.offset(1, 0, 1)] * (fzp - fxp) - g[i.offset(-1, 0, -1)] * (fxm - fzm) -
              g[i.offset(-1, 0, 1)] * (fzp - fxm) + g[i.offset(1, 0, -1)] * (fxp - fzm));
...
int main(int argc, char **argv) {
  ...
  for (auto i : n.region(RGN_NOBNDRY)) {
    ...
    ddt(n)[i] =
        -bracket_arakawa(phi, n, i) + (2. / R_c) * DDZ_C2(n, i) + n[i] * phi[i] / L_par;

    ddt(vort)[i] =
        -bracket_arakawa(phi, vort, i) + (2. / R_c) * DDZ_C2(n, i) + phi[i] / L_par;
  }
  ...
  ddt(n) = -bracket_arakawa(phi, n) // bracket(phi, n, BRACKET_ARAKAWA)
           + (2. / R_c) * DDZ(n) + n * phi / L_par;

  ddt(vort) = -bracket_arakawa(phi, vort) // bracket(phi, vort, BRACKET_ARAKAWA)
              + (2. / R_c) * DDZ(n) + phi / L_par;
