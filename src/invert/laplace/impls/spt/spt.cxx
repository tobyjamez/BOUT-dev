...
const FieldPerp LaplaceSPT::solve(const FieldPerp &b) { return solve(b, b); }
 ...
 * @param[inout] gp   gam from the processor mesh->PE_XIND + 1, and returned to
 * mesh->PE_XIND - 1
 * @param[inout] up   u from processor mesh->PE_XIND + 1, and returned to mesh->PE_XIND -
 * 1
 */
 ...
 * @param[in]    inner_boundary_flags  Inversion settings for inner boundary (see
 * invert_laplace.hxx for values)
 * @param[in]    outer_boundary_flags  Inversion settings for outer boundary (see
 * invert_laplace.hxx for values)
...
int LaplaceSPT::start(const FieldPerp &b, SPT_data &data) {
  ...
  data.allocate(
      mm,
      mesh->LocalNx); // Make sure data is allocated. Already allocated -> does nothing
...
/*!
  ...
  @param[in]    inner_boundary_flags  Inversion flags for inner boundary (same as passed
  to invert_spt_start)
  @param[in]    outer_boundary_flags  Inversion flags for outer boundary (same as passed
  to invert_spt_start)
