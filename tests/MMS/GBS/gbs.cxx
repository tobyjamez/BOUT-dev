...
int GBS::rhs(BoutReal t) {
  ...
  if (evolve_Ne) {
    ...
    if (parallel) {
      ddt(Ne) -=
          Ne * Grad_par(Ve) + Vpar_Grad_par(Ve, Ne); // Parallel compression, advection
    }
  ...
  if (evolve_Ve) {
    ...
    ddt(VePsi) =
        -vE_Grad(Ve, phi) - Vpar_Grad_par(Ve, Ve) - mi_me * (2. / 3.) * Grad_par(Ge) -
        mi_me * nu * (Ve - Vi) + mi_me * Grad_par(phi) -
        mi_me * (Te * Grad_par(logNe) + 1.71 * Grad_par(Te)) + D(Ve, Dve) + H(Ve, Hve);
  }

  if (evolve_Vi) {
    // Ion velocity

    ddt(Vi) = -vE_Grad(Vi, phi) - Vpar_Grad_par(Vi, Vi) - (2. / 3.) * Grad_par(Gi) -
