...
private:
  typedef std::vector<std::basic_ostream<char_type, traits> *> stream_container;
  typedef typename stream_container::iterator iterator;
  stream_container streams_;

public:
  void add(std::basic_ostream<char_type, traits> &str) {
    iterator pos = std::find(streams_.begin(), streams_.end(), &str);
...
protected:
  virtual std::streamsize xsputn(const char_type *sequence, std::streamsize num) {
...
template <typename char_type, typename traits> class multioutbuf_init {
private:
  multioutbuf<char_type, traits> buf_;

public:
  multioutbuf<char_type, traits> *buf() { return &buf_; }
};

template <typename char_type, typename traits = std::char_traits<char_type>>
class multiostream : private multioutbuf_init<char_type, traits>,
