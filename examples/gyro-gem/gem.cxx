...
class GEM : public PhysicsModel {
  ...
  int init(bool restarting) override {
    ...
    Tenorm = max(Te0, true);
    SAVE_ONCE(Tenorm); // Maximum value over the grid
    Ninorm = max(Ni0, true);
    SAVE_ONCE(Ninorm);

    Cs = sqrt(qe * Tenorm / (AA * Mp));
    SAVE_ONCE(Cs); // Sound speed in m/s
    ...
    delta = rho_s / Lbar;
    SAVE_ONCE(delta); // This should be small
    ...
    OPTION6(options, apue_ddt, apue_phi1, apue_pet, apue_curv, apue_gradB, apue_Rei,
            true);
    OPTION4(options, apue_ue1_phi1, apue_qe1_phi1, apue_apar1_phi1, apue_apar1_pe1,
            nonlinear);
    ...
    OPTION6(options, apui_ddt, apui_phi1, apui_pit, apui_curv, apui_gradB, apui_Rei,
            true);
    OPTION4(options, apui_ui1_phi1, apui_qi1_phi1, apui_apar1_phi1, apui_apar1_pi1,
            nonlinear);
    ...
    t_i = pow(ZZ, -4.) * sqrt(AA) /
          (4.80e-8 * (Ninorm / 1e6) * Coulomb * pow(Tenorm, -3. / 2));
  ...
  int convective(BoutReal time) override {
    ...
    if (!adiabatic_electrons) {
      ...
      if (ne_ddt) {
        ...
        if (ne_curv)
          ddt(Ne) += curvature(phi_G + tau_e * Ne +
                               0.5 * (tau_e * Tepar + tau_e * Teperp + Phi_G));
      ...
      if (apue_ddt) {
        ...
        if (apue_pet) // Linear terms
          ddt(ApUe) -=
              tau_i * Grad_parP_CtoL(Ne0 + Te0) + tau_i * Grad_par_CtoL(Ne + Tepar);
      ...
      if (tepar_ddt) {
        ...
        if (nonlinear) {
          ddt(Tepar) += -UE_Grad(Te0 + Tepar, phi_G) - 2. * Div_parP_LtoC(Ue + qepar);
        } else {
          ddt(Tepar) += -UE_Grad(Te0, phi_G) - 2. * Div_par_LtoC(Ue + qepar);
        }
      ...
      if (teperp_ddt) {
        ddt(Teperp) = +0.5 * curvature(phi_G + Phi_G + tau_e * (Ne + Teperp) +
        ...
        if (nonlinear) {
          ...
          ddt(Teperp) += -UE_Grad(Te0, phi_G) - WE_Grad(Ne0 + 2. * Te0, Phi_G) -
                         Div_par_LtoC(qeperp);
        }
      ...
      if (qepar_ddt) {
        ddt(qepar) = -1.5 * (1. / mu_e) * Grad_parP_CtoL(tau_e * Te0) +
                     0.5 * mu_e * tau_e * curvature(3. * Ue + 8. * qepar);

        if (nonlinear) {
          ddt(qepar) +=
              -UE_Grad(qepar, phi_G) - 1.5 * (1. / mu_e) * Grad_parP_CtoL(tau_e * Tepar);
        } else {
          ddt(qepar) += -1.5 * (1. / mu_e) * Grad_par_CtoL(tau_e * Tepar);
        }
      ...
      if (qeperp_ddt) {
        ...
        if (nonlinear) {
          ddt(qeperp) += -UE_Grad(qeperp, phi_G) - WE_Grad(Ue + 2. * qeperp, Phi_G) -
                         (1. / mu_e) * Grad_parP_CtoL(Phi_G + tau_e * Teperp);
        } else {
          ddt(qeperp) += -(1. / mu_e) * Grad_par_CtoL(Phi_G + tau_e * Teperp);
        }
    ...
    if (ni_ddt) {
      ...
      if (ni_curv)
        ddt(Ni) += curvature(phi_G + tau_i * Ni +
                             0.5 * (tau_i * Tipar + tau_i * Tiperp + Phi_G));
    ...
    if (apui_ddt) {
      ...
      if (apui_pit) // Linear terms
        ddt(ApUi) -=
            tau_i * Grad_parP_CtoL(Ni0 + Ti0) + tau_i * Grad_par_CtoL(Ni + Tipar);
    ...
    if (tipar_ddt) {
      ddt(Tipar) = +curvature(phi_G + tau_i * (Ni + Tipar) + 2. * tau_i * Tipar) -
                   (Ui + qiperp) * Grad_par_logB;

      if (nonlinear) {
        ddt(Tipar) += -UE_Grad(Ti0 + Tipar, phi_G) - 2. * Div_parP_LtoC(Ui + qipar);
      } else {
        ddt(Tipar) += -UE_Grad(Ti0, phi_G) - 2. * Div_par_LtoC(Ui + qipar);
      }
    ...
    if (tiperp_ddt) {
      ddt(Tiperp) = +0.5 * curvature(phi_G + Phi_G + tau_i * (Ni + Tiperp) +
      ...
      if (nonlinear) {
        ...
        ddt(Tiperp) +=
            -UE_Grad(Ti0, phi_G) - WE_Grad(Ni0 + 2. * Ti0, Phi_G) - Div_par_LtoC(qiperp);
      }
    ...
    if (qipar_ddt) {
      ...
      if (nonlinear) {
        ddt(qipar) +=
            -UE_Grad(qipar, phi_G) - 1.5 * (1. / mu_i) * Grad_parP_CtoL(tau_i * Tipar);
      } else {
        ddt(qipar) += -1.5 * (1. / mu_i) * Grad_par_CtoL(tau_i * Tipar);
      }
    ...
    if (qiperp_ddt) {
      ...
      if (nonlinear) {
        ddt(qiperp) += -UE_Grad(qiperp, phi_G) - WE_Grad(Ui + 2. * qiperp, Phi_G) -
                       (1. / mu_i) * Grad_parP_CtoL(Phi_G + tau_i * Tiperp);
      } else {
        ddt(qiperp) += -(1. / mu_i) * Grad_par_CtoL(Phi_G + tau_i * Tiperp);
      }
  ...
  int diffusive(BoutReal time) override {
    ...
    Rei = mu_e * nu_e *
          (eta * Jpar + (alpha_e / kappa_e) * (qepar + qeperp + alpha_e * Jpar));
    ...
    if (!adiabatic_electrons) {
      ...
      K_par =
          mu_e * tau_e * nu_e * ((5. / 2.) / kappa_e) * (qepar + 0.6 * alpha_e * Jpar);
      K_perp =
          mu_e * tau_e * nu_e * ((5. / 2.) / kappa_e) * (qeperp + 0.4 * alpha_e * Jpar);
      ...
      if (qepar_ddt) {
        ddt(qepar) = -UE_Grad_D(qepar, phi_G)
                     ...
                     - (1. / mu_e) * K_par - (1. / mu_e) * K_D;
      }
      if (qeperp_ddt) {
        ddt(qeperp) =
            -UE_Grad_D(qeperp, phi_G) - (1. / mu_e) * K_perp + (1. / mu_e) * K_D;
      }
    }
    ...
    if (qipar_ddt) {
      ddt(qipar) = -UE_Grad_D(qipar, phi_G) - (1. / mu_e) * K_par - (1. / mu_e) * K_D;
    }
    if (qiperp_ddt) {
      ddt(qiperp) = -UE_Grad_D(qiperp, phi_G) - (1. / mu_e) * K_perp + (1. / mu_e) * K_D;
    }
  ...
  const Field3D WE_Grad(const Field3D &f, const Field3D &p) { return bracket(p, f, bm); }
