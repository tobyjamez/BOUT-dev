...
const Field3D Div_par(const Field3D &f_in, const Field3D &v_in, const Field3D &a,
                      bool fixflux = true) {
  ...
  for (int i = mesh->xstart; i <= mesh->xend; i++)
    for (int j = mesh->ystart; j <= mesh->yend; j++)
      for (int k = 0; k < mesh->LocalNz; k++) {
        ...
        BoutReal vL = 0.25 * (vx(i - 1, j, k) + vx(i, j, k)) *
                      (coord->J(i - 1, j) + coord->J(i, j));
        BoutReal vR = 0.25 * (vx(i + 1, j, k) + vx(i, j, k)) *
                      (coord->J(i + 1, j) + coord->J(i, j));
  ...
  for (int i = mesh->xstart; i <= mesh->xend; i++)
    for (int j = mesh->ystart; j <= mesh->yend; j++)
      for (int k = 0; k < mesh->LocalNz; k++) {
        ...
        BoutReal vU = 0.25 * (vy(i, j, k) + vy(i, j + 1, k)) *
                      (coord->J(i, j) + coord->J(i, j + 1));
        BoutReal vD = 0.25 * (vy(i, j, k) + vy(i, j - 1, k)) *
                      (coord->J(i, j) + coord->J(i, j - 1));
