...
CubicSpline::~CubicSpline() { delete[] coeffs; }

void CubicSpline::initialise(const char &inputdirection,
                             const bool pass_include_boundary_guard_cells,
                             const bool pass_upper_boundary_offset) {
  direction = inputdirection;
  if (direction == 'y') {

  } else
    throw BoutException("CubicSpline: direction is not valid");
...
void CubicSpline::calculate(const Field3D &pass_input) {
  if (direction == 'y') {
    ...
    if (!include_boundary_guard_cells) {
      ...
      grad_input = mesh->indexDDY(input, input.getLocation(),
                                  DIFF_DEFAULT); // derivative in index space

      // values of grad_input that depended on the guard cells are wrong, replace them
      // with forward/backward derivatives
      for (RangeIterator rlow = mesh->iterateBndryLowerY(); !rlow.isDone(); rlow++)
        for (int jz = 0; jz < mesh->LocalNz; jz++)
          for (int jy = mesh->ystart + 1; jy >= 0; jy--) {
            grad_input(rlow.ind, jy, jz) =
                (-11. * input(rlow.ind, jy, jz) + 18. * input(rlow.ind, jy + 1, jz) -
                 9. * input(rlow.ind, jy + 2, jz) + 2. * input(rlow.ind, jy + 3, jz)) /
                6.;
          }
      for (RangeIterator rup = mesh->iterateBndryUpperY(); !rup.isDone(); rup++)
        for (int jz = 0; jz < mesh->LocalNz; jz++)
          // 	  for (int jy=mesh->yend-1-staggered; jy<mesh->LocalNy; jy++) {
          for (int jy = mesh->yend - 1 - upper_boundary_offset; jy < mesh->LocalNy;
               jy++) {
            grad_input(rup.ind, jy, jz) =
                (11. * input(rup.ind, jy, jz) - 18. * input(rup.ind, jy - 1, jz) +
                 9. * input(rup.ind, jy - 2, jz) - 2. * input(rup.ind, jy - 3, jz)) /
                6.;
          }
    } else {
      grad_input = mesh->indexDDY(input, input.getLocation(),
      ...
      // May need values in the guard cells as well, calculate with forward/backward
      // derivatives
      for (RangeIterator rlow = mesh->iterateBndryLowerY(); !rlow.isDone(); rlow++)
        for (int jz = 0; jz < mesh->LocalNz; jz++)
          for (int jy = mesh->ystart - 1; jy >= 0; jy--) {
...
BoutReal *CubicSpline::coefficients(bindex *position) {
  ...
  coeffs[2] = 3. * (input(position->jx, position->jyp, position->jz) -
                    input(position->jx, position->jy, position->jz)) -
              2. * grad_input(position->jx, position->jy, position->jz) -
              grad_input(position->jx, position->jyp, position->jz);
  coeffs[3] = 2. * (input(position->jx, position->jy, position->jz) -
                    input(position->jx, position->jyp, position->jz)) +
              grad_input(position->jx, position->jy, position->jz) +
              grad_input(position->jx, position->jyp, position->jz);
  return coeffs;
}
...
BoutReal *CubicSpline::coefficients(const int &jx, const int &jy, const int &jz) {
  ...
  coeffs[3] = 2. * (input(jx, jy, jz) - input(jx, jy + 1, jz)) + grad_input(jx, jy, jz) +
              grad_input(jx, jy + 1, jz);
  return coeffs;
}
