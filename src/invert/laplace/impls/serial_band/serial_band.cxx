...
LaplaceSerialBand::LaplaceSerialBand(Options *opt)
    : Laplacian(opt), Acoef(0.0), Ccoef(1.0), Dcoef(1.0) {
  if (!mesh->firstX() || !mesh->lastX())
    throw BoutException("LaplaceSerialBand only works for mesh->NXPE = 1");
  if (mesh->periodicX) {
    throw BoutException("LaplaceSerialBand does not work with periodicity in the x "
...
const FieldPerp LaplaceSerialBand::solve(const FieldPerp &b) { return solve(b, b); }
      ...
      if (nonuniform) {
        ...
        if ((ix != 0) && (ix != ncx))
          coef4 += coord->g11(ix, jy) *
                   ((1.0 / coord->dx(ix + 1, jy)) - (1.0 / coord->dx(ix - 1, jy))) /
                   (2.0 * coord->dx(ix, jy));
      }
