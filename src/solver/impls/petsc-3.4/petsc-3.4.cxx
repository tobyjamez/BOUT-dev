...
// Adding support for 3.4 <DD>

#ifdef BOUT_HAS_PETSC_3_4

#include "petsc-3.4.hxx"
...
int PetscSolver::init(int NOUT, BoutReal TIMESTEP) {
  ...
  if (J_load) {
    ...
    if (J_slowfd) { // create Jacobian matrix by slow fd
      ...
      ierr = ISLocalToGlobalMappingCreate(comm, n, ltog_array, PETSC_COPY_VALUES, &ltog);
      CHKERRQ(ierr);
      ierr = ISLocalToGlobalMappingBlock(ltog, dof, &ltogb);
      CHKERRQ(ierr);
      ...
      CHKERRQ(ierr);
      ierr = MatSetLocalToGlobalMappingBlock(J, ltogb, ltogb);
      CHKERRQ(ierr);
      ...
      CHKERRQ(ierr);
      ierr = ISLocalToGlobalMappingDestroy(&ltogb);
      CHKERRQ(ierr);
    }
  }
