...
LaplacePetsc::LaplacePetsc(Options *opt)
  ...
  if (fourth_order) {
    // first and last 2*mesh-LocalNz entries are the edge x-values that (may) have
    // 'off-diagonal' components (i.e. on another processor)
    ...
    // first and last mesh-LocalNz entries are the edge x-values that (may) have
    // 'off-diagonal' components (i.e. on another processor)
    if (mesh->firstX() && mesh->lastX()) {
      for (int i = 0; i < mesh->LocalNz; i++) {
        d_nnz[i] = 6;
  ...
  // WARNING: only a few of these options actually make sense: see the PETSc documentation
  // to work out which they are (possibly pbjacobi, sor might be useful choices?)
  string pctypeoption;
...
const FieldPerp LaplacePetsc::solve(const FieldPerp &b) { return solve(b, b); }
 ...
 *                  May also contain the  boundary condition if flag 32 - INVERT_SET is
 * set
 *
...
const FieldPerp LaplacePetsc::solve(const FieldPerp &b, const FieldPerp &x0) {
  ...
  int i = Istart; // The row in the PETSc matrix
  {
    ...
    //     if ((fourth_order) && !(lastflag&INVERT_4TH_ORDER)) throw BoutException("Should
    //     not change INVERT_4TH_ORDER flag in LaplacePetsc: 2nd order and 4th order
    //     require different pre-allocation to optimize PETSc solver");
        ...
        for (int z = 0; z < mesh->LocalNz; z++) {
          ...
          if (inner_boundary_flags & INVERT_AC_GRAD) {
            ...
            if (fourth_order) {
              ...
              Element(i, x, z, 0, 0,
                      -25.0 / (12.0 * coord->dx(x, y)) / sqrt(coord->g_11(x, y)), MatA);
              Element(i, x, z, 1, 0, 4.0 / coord->dx(x, y) / sqrt(coord->g_11(x, y)),
                      MatA);
              Element(i, x, z, 2, 0, -3.0 / coord->dx(x, y) / sqrt(coord->g_11(x, y)),
                      MatA);
              Element(i, x, z, 3, 0,
                      4.0 / (3.0 * coord->dx(x, y)) / sqrt(coord->g_11(x, y)), MatA);
              Element(i, x, z, 4, 0,
                      -1.0 / (4.0 * coord->dx(x, y)) / sqrt(coord->g_11(x, y)), MatA);
            } else {
              ...
              //                    );
              // //                   Element(i,x,z, 3, 0, 0.0, MatA );  // Reset these
              // elements to 0 in case 4th order flag was used previously: not allowed now
              // //                   Element(i,x,z, 4, 0, 0.0, MatA );
              // Second Order Accuracy on Boundary, set half-way between grid points
              Element(i, x, z, 0, 0, -1.0 / coord->dx(x, y) / sqrt(coord->g_11(x, y)),
                      MatA);
              Element(i, x, z, 1, 0, 1.0 / coord->dx(x, y) / sqrt(coord->g_11(x, y)),
                      MatA);
              Element(i, x, z, 2, 0, 0.0, MatA);
              //                      Element(i,x,z, 3, 0, 0.0, MatA );  // Reset these
              //                      elements to 0 in case 4th order flag was used
              //                      previously: not allowed now
              //                      Element(i,x,z, 4, 0, 0.0, MatA );
            }
          } else {
            if (fourth_order) {
        ...
        for (int z = 0; z < mesh->LocalNz; z++) {
          ...
          if (outer_boundary_flags & INVERT_AC_GRAD) {
            ...
            if (fourth_order) {
              ...
              //                    // Second Order Accuracy on Boundary
              //                    Element(i,x,z,  0, 0,  3.0 / (2.0*coord->dx(x,y)),
              //                    MatA );
              //                    Element(i,x,z, -1, 0, -2.0 / coord->dx(x,y), MatA );
              //                    Element(i,x,z, -2, 0,  1.0 / (2.0*coord->dx(x,y)),
              //                    MatA );
              // //                   Element(i,x,z, -3, 0,  0.0, MatA );  // Reset these
              // elements to 0 in case 4th order flag was used previously: not allowed now
              // //                   Element(i,x,z, -4, 0,  0.0, MatA );
...
void LaplacePetsc::Coeffs(int x, int y, int z, BoutReal &coef1, BoutReal &coef2,
  ...
  if (mesh->IncIntShear) {
    // d2dz2 term
    coef2 +=
        coord->g11(x, y) * coord->IntShiftTorsion(x, y) * coord->IntShiftTorsion(x, y);
    // Mixed derivative
