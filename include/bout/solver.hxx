...
public:
  ...
  void addTimestepMonitor(
      TimestepMonitorFunc f); ///< Add a monitor function to be called every timestep
  void removeTimestepMonitor(
      TimestepMonitorFunc f); ///< Remove a previously added timestep monitor
...
protected:
  ...
  template <class T> struct VarStr {
    bool constraint;
    T *var;
    T *F_var;
  ...
  int run_diffusive(BoutReal t,
                    bool linear = true); ///< Calculate only the diffusive parts

  int call_monitors(BoutReal simtime, int iter,
                    int NOUT); ///< Calls all monitor functions
...
private:
  ...
  int freqDefault =
      1; ///< Default sampling rate at which to call monitors - same as output to screen
  ...
  std::list<TimestepMonitorFunc>
      timestep_monitors; ///< List of timestep monitor functions
