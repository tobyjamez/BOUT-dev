...
public:
  typedef T data_type;
  Matrix() : n1(0), n2(0){};
  Matrix(unsigned int n1, unsigned int n2) : n1(n1), n2(n2) { data = Array<T>(n1 * n2); }
...
template <typename T> DEPRECATED(void free_matrix(Matrix<T> UNUSED(m)));
template <typename T> void free_matrix(Matrix<T> UNUSED(m)){};
...
template <class T> DEPRECATED(T **matrix(int xsize, int ysize));
 ...
 */
template <class T> T **matrix(int xsize, int ysize) {
  long i;
  T **m;
...
template <class T> DEPRECATED(void free_matrix(T **m));
/*!
 ...
 * using a > b operator
 */
template <typename T> T BOUTMAX(T a) { return a; }
template <typename T, typename... Args> T BOUTMAX(T a, T b, Args... args) {
  T c = BOUTMAX(b, args...);
  return c > a ? c : a;
}
 ...
 */
template <typename T> T BOUTMIN(T a) { return a; }
template <typename T, typename... Args> T BOUTMIN(T a, T b, Args... args) {
  T c = BOUTMIN(b, args...);
  return c < a ? c : a;
}
 ...
 * by testing if a > 0
 */
template <typename T> T SIGN(T a) { // Return +1 or -1 (0 -> +1)
  return a < 0 ? -1 : +1;
}
 ...
 */
template <class T> const string toString(const T &val) {
  std::stringstream ss;
  ss << val;
  return ss.str();
}
