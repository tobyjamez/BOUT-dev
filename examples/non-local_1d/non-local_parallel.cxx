...
void NonLocalParallel::initialise(BoutReal pass_electron_charge,
                                  BoutReal pass_electron_mass, BoutReal pass_ion_mass,
                                  BoutReal pass_epsilon_0, BoutReal pass_logLambda,
                                  const bool pass_fluxes_location_is_ylow,
                                  BoutReal pass_gamma_factor) {
  ...
  boundary_gradient_smoothing_length =
      mesh->GlobalNy / 256; // Number of grid points to average over for the gradient used
                            // to extrapolate to the guard cells
  boundary_condition_smoothing_range =
      BoutReal(mesh->GlobalNy) / 64.; // Scale length of the exponential decay used to
                                      // smoothly apply the boundary condition
  if (boundary_gradient_smoothing_length == 0)
    boundary_gradient_smoothing_length = 1;

  if (fluxes_location_is_ylow && !mesh->StaggerGrids)
    throw BoutException("Trying to calculate the heat flux at CELL_YLOW while "
                        "StaggerGrids=false is an error.");

  cubic_spline_inverse_lambdaC.initialise('y', true, fluxes_location_is_ylow);
  if (fluxes_location_is_ylow) {
#ifdef DRIVE_GRADT
    cubic_spline_gradT_driveterm.initialise(
        'y', true,
        true); // CELL_CENTRE quantity, so must be adjusted when the grids are staggered.
    cubic_spline_gradT.initialise('y', true, false); // will fix up the boundary guard
                                                     // cells with forward/backward
                                                     // derivatives, since at least one
                                                     // guard cell value will be needed
#endif
#ifdef DRIVE_GRADV
...
#ifdef DRIVE_GRADT
    cubic_spline_gradT_driveterm.initialise(
        'y', false, false); // false because gradT_driveterm includes a derivative, so we
                            // don't know it in the guard cells, hence calculate the
                            // interpolation excluding the guard cells at the target
                            // boundaries.
#endif
#ifdef DRIVE_GRADV
    cubic_spline_gradV_driveterm.initialise(
...
#endif
#ifdef CALCULATE_VISCOSITY
    electron_viscosity.setLocation(CELL_YLOW); // should really have this as CELL_CENTRE
                                               // quantity, but then would have to
                                               // calculate the integrals on both
                                               // CELL_CENTRE and CELL_YLOW which is
                                               // perfectly possible but has not been
                                               // implemented yet
#endif
#ifdef CALCULATE_FRICTION
...
#endif
#ifdef DRIVE_GRADV
  heatflux_infile >> junk; // the zero eigenvector has only odd-l components, so the gradV
                           // drive (which is (2,0)) does not contribute
#else
  heatflux_infile >> junk;
#endif
#ifdef DRIVE_VEMINUSVI
...
#endif
#ifdef DRIVE_GRADT
  viscosity_infile >> junk; // the zero eigenvector has only odd-l components, so does not
                            // contribute to the viscosity (2,0)
#else
...
#define ALLOCATED_EIGENVALUES
...
#endif
#ifdef DRIVE_GRADV
  friction_infile >> junk; // the zero eigenvector has only odd-l components, so the gradV
                           // drive (which is (2,0)) does not contribute
#else
  friction_infile >> junk;
#endif
  ...
  if (is_lower_boundary) {
    exp_total_dimensionless_length_over_eigenvalue =
        new BoutReal[number_of_negative_eigenvalues];
  }
...
#endif
#ifdef BC_HEATFLUX
  heatflux_transients_factors =
      new BoutReal[2 * (mesh->xend - mesh->xstart + 1) * (mesh->LocalNz)];
#endif
#ifdef CALCULATE_VISCOSITY
...
void NonLocalParallel::calculate_nonlocal_closures(
    const Field3D &n_electron, const Field3D &T_electron
#ifdef DRIVE_GRADV
    ,
...
void NonLocalParallel::calculate_nonlocal_closures_cell_centre(
...
#ifdef DRIVE_GRADT
  gradT_driveterm = 5. / 4. * n_electron / T_electron * Grad_par(T_electron) /
                    lambdaC_inverse; // g^(1,1)
  mesh->communicate(gradT_driveterm);
#endif
#ifdef DRIVE_GRADV
  gradV_driveterm = -0.5 * n_electron / sqrt(2. * T_electron / electron_mass) *
                    Grad_par(V_electron) / lambdaC_inverse;
  mesh->communicate(gradV_driveterm);
#endif
#ifdef DRIVE_VEMINUSVI
  VeminusVi_driveterm =
      -2. / sqrt(PI) * (-jpar / electron_charge) / sqrt(2. * T_electron / electron_mass);
  ...
  if (mesh->UpXSplitIndex() != 0 || mesh->DownXSplitIndex() != 0)
    throw BoutException("This code cannot currently handle x-splitting of processors.");
  if (!is_lower_boundary) {
    FieldPerp pass_dimensionless_length;
    pass_dimensionless_length.allocate();
    {
      mesh->wait(mesh->irecvYInOutdest(
          *pass_dimensionless_length.getData(), mesh->LocalNx * (mesh->LocalNz),
  ...
  do {
    ...
    dimensionless_length_deltas_above[*position] /* = dy/dt*(a + 1/2*b + 1/3*c + 1/4*d) */
        = coord->dy(position->jx, position->jy) *
          ...
          (interp_coefficients[0] + interp_coefficients[1] / 2. +
           interp_coefficients[2] / 3. + interp_coefficients[3] / 4.);
    ...
    do {
      ...
      dimensionless_length_deltas_above[*position] /* = dy/dt*(a + 1/2*b + 1/3*c + 1/4*d)
          ...
          = coord->dy(position->jx, position->jy) *
            ...
            (interp_coefficients[0] + interp_coefficients[1] / 2. +
             interp_coefficients[2] / 3. + interp_coefficients[3] / 4.);
      increasing_dimensionless_length(position->jx, position->jyp, position->jz) =
          increasing_dimensionless_length[*position] +
          dimensionless_length_deltas_above[*position];
  ...
  {
    Timer timer("comms");
    mesh->sendYOutOutdest(
        *increasing_dimensionless_length.slice(mesh->yend + 1).getData(),
        mesh->LocalNx * (mesh->LocalNz),
        NONLOCAL_PARALLEL_TAGBASE + position->jx * mesh->LocalNz + position->jz);
  }

  // Send the total dimensionless_length at the upper boundary back to the other
  // processors.
  if (is_upper_boundary)
    total_dimensionless_length = increasing_dimensionless_length.slice(mesh->yend);

  y_broadcast(*total_dimensionless_length.getData(), mesh->LocalNx * mesh->LocalNz,
              mesh->getNYPE() - 1);
...
#ifdef DRIVE_GRADT
#ifdef CALCULATE_HEATFLUX
  electron_heat_flux += -heatflux_gradT_zerocoeff *
                        gradT_driveterm; // zero eigenvalue contribution to n^(1,1) //zero
                                         // eigenvalue contribution to n^(1,1)/T^1.
#endif
...
// viscosity gets no zero eigenvalue contribution
  ...
  for (int i = 0; i < number_of_negative_eigenvalues; i++) {
#ifdef DRIVE_GRADT
    integration.calculateIntegralBelow_cell_centre(
        eigenvalues[i], dimensionless_length_deltas_above, cubic_spline_inverse_lambdaC,
        cubic_spline_gradT_driveterm, i + 0);
#ifdef CALCULATE_HEATFLUX
    ...
    integration.calculateIntegralBelow_cell_centre(
        eigenvalues[i], dimensionless_length_deltas_above, cubic_spline_inverse_lambdaC,
        cubic_spline_gradV_driveterm, i + number_of_negative_eigenvalues);
#ifdef CALCULATE_HEATFLUX
    ...
    integration.calculateIntegralBelow_cell_centre(
        eigenvalues[i], dimensionless_length_deltas_above, cubic_spline_inverse_lambdaC,
        cubic_spline_VeminusVi_driveterm, i + 2 * number_of_negative_eigenvalues);
#ifdef CALCULATE_HEATFLUX
    electron_heat_flux += heatflux_VeminusVi_coefficients[i] * integration.integral_below;
#endif
#ifdef CALCULATE_VISCOSITY
    electron_viscosity +=
        viscosity_VeminusVi_coefficients[i] * integration.integral_below;
#endif
#ifdef CALCULATE_FRICTION
  ...
  for (int i = 0; i < number_of_negative_eigenvalues; i++) {
#ifdef DRIVE_GRADT
    integration.calculateIntegralAbove_cell_centre(
        eigenvalues[i], dimensionless_length_deltas_above, cubic_spline_inverse_lambdaC,
        cubic_spline_gradT_driveterm, i + 0);
#ifdef CALCULATE_HEATFLUX
    electron_heat_flux +=
        -heatflux_gradT_coefficients[i] *
        integration.integral_above; // gives \hat(n)^A //gives \hat(n)^A/(T_electron^1.5)
#endif
#ifdef CALCULATE_VISCOSITY
...
#ifdef CALCULATE_HEATFLUX
    electron_heat_flux +=
        -heatflux_gradV_coefficients[i] * integration.integral_above; // gives \hat(n)^A
#endif
#ifdef CALCULATE_VISCOSITY
...
#ifdef CALCULATE_HEATFLUX
    electron_heat_flux += -heatflux_VeminusVi_coefficients[i] *
                          integration.integral_above; // gives \hat(n)^A
#endif
#ifdef CALCULATE_VISCOSITY
    electron_viscosity +=
        -viscosity_VeminusVi_coefficients[i] * integration.integral_above;
#endif
#ifdef CALCULATE_FRICTION
  ...
  //   throw BoutException("cell_centre version of boundary conditions code needs
  //   checking, at the moment it has just been cut&pasted from cell_ylow");
...
#ifdef BC_HEATFLUX
      pass_interim_upper_boundary_n11[rup.ind][jz] =
          electron_heat_flux[rup.ind][mesh->yend][jz];
      upper_boundary_condition_n11[rup.ind][jz] =
          -4. / 5. * sqrt(electron_mass / 2.) / pow(Te_here, 1.5) *
          heat_flux_boundary_condition[rup.ind][mesh->yend][jz];
#endif
#ifdef BC_VISCOSITY
      pass_interim_upper_boundary_n20[rup.ind][jz] =
          electron_viscosity[rup.ind][mesh->yend][jz];
      upper_boundary_condition_n20[rup.ind][jz] =
          viscosity_boundary_condition[rup.ind][mesh->yend][jz] / Te_here;
#endif
    }
  if (is_upper_boundary && mesh->getNYPE() > 1) {
#ifdef BC_HEATFLUX
    MPI_Request request1 = mesh->sendToProc(
        mesh->getXProcIndex(), 0, *pass_interim_upper_boundary_n11.getData(),
        mesh->LocalNx * mesh->LocalNz, NONLOCAL_PARALLEL_TAGBASE + mesh->getXProcIndex());
    MPI_Request request2 = mesh->sendToProc(
        mesh->getXProcIndex(), 0, *upper_boundary_condition_n11.getData(),
        mesh->LocalNx * mesh->LocalNz,
...
#endif
#ifdef BC_VISCOSITY
    MPI_Request request3 = mesh->sendToProc(
        mesh->getXProcIndex(), 0, *pass_interim_upper_boundary_n20.getData(),
        mesh->LocalNx * mesh->LocalNz,
        NONLOCAL_PARALLEL_TAGBASE + mesh->getXProcIndex() + 3);
    MPI_Request request4 = mesh->sendToProc(
        mesh->getXProcIndex(), 0, *upper_boundary_condition_n20.getData(),
        mesh->LocalNx * mesh->LocalNz,
  ...
  if (is_lower_boundary && mesh->getNYPE() > 1) {
    ...
    mesh->wait(mesh->receiveFromProc(
        mesh->getXProcIndex(), mesh->getNYPE() - 1,
        *upper_boundary_condition_n11.getData(), mesh->LocalNx * mesh->LocalNz,
        NONLOCAL_PARALLEL_TAGBASE + mesh->getXProcIndex() + 1));
#endif
#ifdef BC_VISCOSITY
    mesh->wait(mesh->receiveFromProc(
        mesh->getXProcIndex(), mesh->getNYPE() - 1,
        *pass_interim_upper_boundary_n20.getData(), mesh->LocalNx * mesh->LocalNz,
        NONLOCAL_PARALLEL_TAGBASE + mesh->getXProcIndex() + 3));
    mesh->wait(mesh->receiveFromProc(
        mesh->getXProcIndex(), mesh->getNYPE() - 1,
        *upper_boundary_condition_n20.getData(), mesh->LocalNx * mesh->LocalNz,
        NONLOCAL_PARALLEL_TAGBASE + mesh->getXProcIndex() + 4));
#endif
  }
  ...
  if (is_lower_boundary) {
    for (int jx = mesh->xstart; jx <= mesh->xend; jx++)
      for (int jz = 0; jz < mesh->LocalNz; jz++) {
        ...
        BoutReal lower_boundary_n11 = -4. / 5. * sqrt(electron_mass / 2.) /
                                      pow(Te_here, 1.5) *
                                      heat_flux_boundary_condition[jx][mesh->ystart][jz];
        ...
        Thus the remainder must come from nhat_minus, which we will construct here just to
        give the right 1,1 component (could set number_of_negative_eigenvalues-1 more
        components if desired)
        However, the transients from the other boundary do not necessarily decay to
        vanishing by the time they get to this boundary, so we must solve for both.
        Fortunately this reduces to a single algebraic equation which makes it
        surprisingly easy to do (in the case of just a single condition being imposed at
        least).
        */

        BoutReal sum_decayed_W11_W11_term = 0.;
        for (int i = 0; i < number_of_negative_eigenvalues; i++) {
          exp_total_dimensionless_length_over_eigenvalue[i] =
              exp(total_dimensionless_length[jx][jz] / eigenvalues[i]);
          sum_decayed_W11_W11_term += W11_B_times_WinverseB_11[i] *
                                      exp_total_dimensionless_length_over_eigenvalue[i];
        }
        heatflux_transients_factors[(jx - mesh->xstart) * (mesh->LocalNz) + jz] =
            ((lower_boundary_n11 - interim_lower_boundary_n11) * W11_dot_W11 -
             sum_decayed_W11_W11_term *
                 (upper_boundary_n11 - interim_upper_boundary_n11)) /
            (pow(W11_dot_W11, 2) - pow(sum_decayed_W11_W11_term, 2));
        heatflux_transients_factors[(mesh->xend - mesh->xstart + 1) * (mesh->LocalNz) +
                                    (jx - mesh->xstart) * (mesh->LocalNz) + jz] =
            ((upper_boundary_n11 - interim_upper_boundary_n11) * W11_dot_W11 -
             sum_decayed_W11_W11_term *
                 (lower_boundary_n11 - interim_lower_boundary_n11)) /
            (pow(W11_dot_W11, 2) - pow(sum_decayed_W11_W11_term, 2));
#elif defined(BC_VISCOSITY) && !defined(BC_HEATFLUX)
        BoutReal lower_boundary_n20 =
            viscosity_boundary_condition[jx][mesh->ystart][jz] / Te_here;
        ...
        Thus the remainder must come from nhat_minus, which we will construct here just to
        give the right 2,0 component (could set number_of_negative_eigenvalues-1 more
        components if desired)
        However, the transients from the other boundary do not necessarily decay to
        vanishing by the time they get to this boundary, so we must solve for both.
        Fortunately this reduces to a single algebraic equation which makes it
        ...
        */
        ...
        for (int i = 0; i < number_of_negative_eigenvalues; i++) {
          exp_total_dimensionless_length_over_eigenvalue[i] =
              exp(total_dimensionless_length[jx][jz] / eigenvalues[i]);
          sum_decayed_W20_W20_term += W20_B_times_WinverseB_20[i] *
                                      exp_total_dimensionless_length_over_eigenvalue[i];
        }
        viscosity_transients_factors[(jx - mesh->xstart) * (mesh->LocalNz) + jz] =
            ((lower_boundary_n20 - interim_lower_boundary_n20) * W20_dot_W20 -
             sum_decayed_W20_W20_term *
                 (upper_boundary_n20 - interim_upper_boundary_n20)) /
            (pow(W20_dot_W20, 2) - pow(sum_decayed_W20_W20_term, 2));
        viscosity_transients_factors[(mesh->xend - mesh->xstart + 1) * (mesh->LocalNz) +
                                     (jx - mesh->xstart) * (mesh->LocalNz) + jz] =
            ((upper_boundary_n20 - interim_upper_boundary_n20) * W20_dot_W20 -
             sum_decayed_W20_W20_term *
                 (lower_boundary_n20 - interim_lower_boundary_n20)) /
            (pow(W20_dot_W20, 2) - pow(sum_decayed_W20_W20_term, 2));
        ...
        for (int i = 0; i < number_of_negative_eigenvalues; i++) {
          ...
          sum_decayed_W20_W20_term += W20_B_times_WinverseB_20[i] *
                                      exp_total_dimensionless_length_over_eigenvalue[i];
          sum_decayed_W20_W11_term += W20_B_times_WinverseB_11[i] *
                                      exp_total_dimensionless_length_over_eigenvalue[i];
          sum_decayed_W11_W20_term += W11_B_times_WinverseB_20[i] *
                                      exp_total_dimensionless_length_over_eigenvalue[i];
        }
        BoutReal det =
            pow(sum_decayed_W11_W20_term, 2) * pow(sum_decayed_W20_W11_term, 2) -
            2 * sum_decayed_W11_W11_term * sum_decayed_W11_W20_term *
                sum_decayed_W20_W11_term * sum_decayed_W20_W20_term +
            pow(sum_decayed_W11_W11_term, 2) * pow(sum_decayed_W20_W20_term, 2) -
            pow(sum_decayed_W20_W20_term, 2) * pow(W11_dot_W11, 2) +
            2 * sum_decayed_W20_W11_term * sum_decayed_W20_W20_term * W11_dot_W11 *
                W11_dot_W20 -
            pow(sum_decayed_W20_W11_term, 2) * pow(W11_dot_W20, 2) -
            2 * sum_decayed_W11_W20_term * sum_decayed_W20_W20_term * W11_dot_W11 *
                W20_dot_W11 +
            2 * sum_decayed_W11_W11_term * sum_decayed_W20_W20_term * W11_dot_W20 *
                W20_dot_W11 -
            pow(sum_decayed_W11_W20_term, 2) * pow(W20_dot_W11, 2) +
            pow(W11_dot_W20, 2) * pow(W20_dot_W11, 2) +
            2 * sum_decayed_W11_W20_term * sum_decayed_W20_W11_term * W11_dot_W11 *
                W20_dot_W20 -
            2 * sum_decayed_W11_W11_term * sum_decayed_W20_W11_term * W11_dot_W20 *
                W20_dot_W20 +
            2 * sum_decayed_W11_W11_term * sum_decayed_W11_W20_term * W20_dot_W11 *
                W20_dot_W20 -
        ...
        heatflux_transients_factors[(jx - mesh->xstart) * (mesh->LocalNz) + jz] =
            ((upper_boundary_n11 - interim_upper_boundary_n11) *
                 (-sum_decayed_W11_W20_term * sum_decayed_W20_W11_term *
                      sum_decayed_W20_W20_term +
             ...
             (upper_boundary_n20 - interim_upper_boundary_n20) *
                 (pow(sum_decayed_W11_W20_term, 2) * sum_decayed_W20_W11_term -
                  sum_decayed_W11_W11_term * sum_decayed_W11_W20_term *
                      sum_decayed_W20_W20_term +
             ...
             (lower_boundary_n11 - interim_lower_boundary_n11) *
                 (-pow(sum_decayed_W20_W20_term, 2) * W11_dot_W11 +
        ...
        viscosity_transients_factors[(jx - mesh->xstart) * (mesh->LocalNz) + jz] =
            ((upper_boundary_n11 - interim_upper_boundary_n11) *
                 (sum_decayed_W11_W20_term * pow(sum_decayed_W20_W11_term, 2) -
                  sum_decayed_W11_W11_term * sum_decayed_W20_W11_term *
                      sum_decayed_W20_W20_term -
             ...
             (lower_boundary_n20 - interim_lower_boundary_n20) *
                 (sum_decayed_W11_W20_term * sum_decayed_W20_W11_term * W11_dot_W11 -
...
#ifdef BC_HEATFLUX
  y_broadcast(heatflux_transients_factors,
              (mesh->xend - mesh->xstart + 1) * (mesh->LocalNz) * 2, 0);
#endif
#ifdef BC_VISCOSITY
  y_broadcast(viscosity_transients_factors,
              (mesh->xend - mesh->xstart + 1) * (mesh->LocalNz) * 2, 0);
#endif
  ...
  for (int jx = mesh->xstart; jx <= mesh->xend; jx++)
    for (int jz = 0; jz < mesh->LocalNz; jz++) {
      ...
      for (int i = 0; i < number_of_negative_eigenvalues; i++) {
#ifdef CALCULATE_HEATFLUX
        heatflux_lower_boundary_transients[i][jx][jz] =
            W11_B_times_WinverseB_11[i] *
            heatflux_transients_factors[(jx - mesh->xstart) * (mesh->LocalNz) + jz];
        heatflux_upper_boundary_transients[i][jx][jz] =
            W11_B_times_WinverseB_11[i] *
            heatflux_transients_factors[(mesh->xend - mesh->xstart + 1) *
                                            (mesh->LocalNz) +
                                        (jx - mesh->xstart) * (mesh->LocalNz) + jz];
#endif
#ifdef CALCULATE_VISCOSITY
        viscosity_lower_boundary_transients[i][jx][jz] =
            W20_B_times_WinverseB_11[i] *
            heatflux_transients_factors[(jx - mesh->xstart) * (mesh->LocalNz) + jz];
        viscosity_upper_boundary_transients[i][jx][jz] =
            -W20_B_times_WinverseB_11[i] *
            heatflux_transients_factors[(mesh->xend - mesh->xstart + 1) *
                                            (mesh->LocalNz) +
                                        (jx - mesh->xstart) * (mesh->LocalNz) + jz];
#endif
#ifdef CALCULATE_FRICTION
        friction_lower_boundary_transients[i][jx][jz] =
            C10_1k_dot_W1k_B_times_WinverseB_11[i] *
            heatflux_transients_factors[(jx - mesh->xstart) * (mesh->LocalNz) + jz];
        friction_upper_boundary_transients[i][jx][jz] =
            C10_1k_dot_W1k_B_times_WinverseB_11[i] *
            heatflux_transients_factors[(mesh->xend - mesh->xstart + 1) *
                                            (mesh->LocalNz) +
                                        (jx - mesh->xstart) * (mesh->LocalNz) + jz];
...
#ifdef CALCULATE_HEATFLUX
        heatflux_lower_boundary_transients[i][jx][jz] =
            W11_B_times_WinverseB_20[i] *
            viscosity_transients_factors[(jx - mesh->xstart) * (mesh->LocalNz) + jz];
        heatflux_upper_boundary_transients[i][jx][jz] =
            -W11_B_times_WinverseB_20[i] *
            viscosity_transients_factors[(mesh->xend - mesh->xstart + 1) *
                                             (mesh->LocalNz) +
                                         (jx - mesh->xstart) * (mesh->LocalNz) + jz];
#endif
#ifdef CALCULATE_VISCOSITY
        viscosity_lower_boundary_transients[i][jx][jz] =
            W20_B_times_WinverseB_20[i] *
            viscosity_transients_factors[(jx - mesh->xstart) * (mesh->LocalNz) + jz];
        viscosity_upper_boundary_transients[i][jx][jz] =
            W20_B_times_WinverseB_20[i] *
            viscosity_transients_factors[(mesh->xend - mesh->xstart + 1) *
...
#ifdef CALCULATE_FRICTION
        friction_lower_boundary_transients[i][jx][jz] =
            C10_1k_dot_W1k_B_times_WinverseB_20[i] *
            viscosity_transients_factors[(jx - mesh->xstart) * (mesh->LocalNz) + jz];
        friction_upper_boundary_transients[i][jx][jz] =
            -C10_1k_dot_W1k_B_times_WinverseB_20[i] *
            viscosity_transients_factors[(mesh->xend - mesh->xstart + 1) *
  ...
  do {
    ...
    do {
      for (int i = 0; i < number_of_negative_eigenvalues; i++) {
        BoutReal exp_increasing =
            exp(increasing_dimensionless_length[*position] / eigenvalues[i]);
        BoutReal exp_decreasing =
            exp(decreasing_dimensionless_length[*position] / eigenvalues[i]);
#ifdef CALCULATE_HEATFLUX
        electron_heat_flux[*position] +=
            heatflux_lower_boundary_transients[i][position->jx][position->jz] *
                exp_increasing +
            heatflux_upper_boundary_transients[i][position->jx][position->jz] *
                exp_decreasing;
#endif
#ifdef CALCULATE_VISCOSITY
        electron_viscosity[*position] +=
            viscosity_lower_boundary_transients[i][position->jx][position->jz] *
                exp_increasing +
            viscosity_upper_boundary_transients[i][position->jx][position->jz] *
                exp_decreasing;
#endif
#ifdef CALCULATE_FRICTION
        electron_friction[*position] +=
            friction_lower_boundary_transients[i][position->jx][position->jz] *
                exp_increasing +
            friction_upper_boundary_transients[i][position->jx][position->jz] *
                exp_decreasing;
#endif
      }
      position->jy++;
...
#ifdef CALCULATE_HEATFLUX
  electron_heat_flux *=
      -5. / 4. * sqrt(2. / electron_mass) *
      (T_electron ^ 1.5); // now we have q=-5/4*v_Telectron*T_electron*n^(1,1)
  ...
  electron_friction += -2. * sqrt(2. * electron_mass * T_electron) * lambdaC_inverse *
                       (-jpar / electron_charge); // Need to include also the friction due
                                                  // directly to the Maxwellian part of
...
#ifdef DRIVE_GRADV
  gradV_driveterm = -0.5 * n_electron / sqrt(2. * T_electron / electron_mass) *
                    Grad_par(V_electron, CELL_CENTRE) / lambdaC_inverse;
//     gradV_electron = Grad_par(V_electron,CELL_YLOW); // would be more consistent to put
//     this on CELL_CENTRE, but that would make imposing a boundary condition a pain.
#endif
#ifdef DRIVE_VEMINUSVI
  VeminusVi_driveterm =
      -2. / sqrt(PI) * (-1. / electron_charge) / sqrt(2. * T_electron / electron_mass);
// jpar is already CELL_YLOW.
#endif
  // Calculate target boundary guard cell derivitives (at YLOW) for gradT_electron with
  // 4th order forward/backward differences from T_electron (at CENTRE)
...
#ifdef DRIVE_GRADT
        gradT_electron(rlow.ind, jy, jz) =
            (-93. * T_electron(rlow.ind, jy, jz) +
             229. * T_electron(rlow.ind, jy + 1, jz) -
             225. * T_electron(rlow.ind, jy + 2, jz) +
             111. * T_electron(rlow.ind, jy + 3, jz) -
             22. * T_electron(rlow.ind, jy + 4, jz)) /
            48. / coord->dy(rlow.ind, jy) /
            sqrt((coord->g_22(rlow.ind, jy) + coord->g_22(rlow.ind, jy + 1) +
                  coord->g_22(rlow.ind, jy + 2) + coord->g_22(rlow.ind, jy + 3) +
                  coord->g_22(rlow.ind, jy + 4)) /
                 5.);
        if (abs(gradT_driveterm(rlow.ind, jy, jz)) > 1.e37 ||
            gradT_driveterm(rlow.ind, jy, jz) != gradT_driveterm(rlow.ind, jy, jz))
          gradT_driveterm(rlow.ind, jy, jz) = 1.e37;
#endif
#ifdef DRIVE_GRADV
// Nothing to be done here: gradV_driveterm is CELL_CENTRE and the guard cell values are
// not used
#endif
#ifdef DRIVE_VEMINUSVI
        if (abs(VeminusVi_driveterm[rlow.ind][jy][jz]) > 1.e37 ||
            VeminusVi_driveterm[rlow.ind][jy][jz] !=
                VeminusVi_driveterm[rlow.ind][jy][jz])
          VeminusVi_driveterm[rlow.ind][jy][jz] = 1.e37;
#endif
        if (abs(lambdaC_inverse[rlow.ind][jy][jz]) > 1.e37 ||
            lambdaC_inverse[rlow.ind][jy][jz] != lambdaC_inverse[rlow.ind][jy][jz])
          lambdaC_inverse[rlow.ind][jy][jz] = 1.e37;
      }
    }
  ...
  for (RangeIterator rup = mesh->iterateBndryUpperY(); !rup.isDone(); rup++)
    for (int jz = 0; jz < mesh->LocalNz; jz++) {
#ifdef DRIVE_GRADT
      gradT_electron[rup.ind][mesh->yend][jz] =
          (T_electron[rup.ind][mesh->yend - 2][jz] -
           27. * T_electron[rup.ind][mesh->yend - 1][jz] +
           27. * T_electron[rup.ind][mesh->yend][jz] -
           T_electron[rup.ind][mesh->yend + 1][jz]) /
          24. / coord->dy[rup.ind][mesh->yend + 1] /
          sqrt((coord->g_22[rup.ind][mesh->yend - 1] + coord->g_22[rup.ind][mesh->yend] +
                coord->g_22[rup.ind][mesh->yend + 1] +
                coord->g_22[rup.ind][mesh->yend + 2]) /
               4.);
#endif
      for (int jy = mesh->yend + 1; jy < mesh->LocalNy; jy++) {
#ifdef DRIVE_GRADT
        gradT_electron[rup.ind][jy][jz] =
            (93. * T_electron(rup.ind, jy - 1, jz) -
             229. * T_electron(rup.ind, jy - 2, jz) +
             225. * T_electron(rup.ind, jy - 3, jz) -
             111. * T_electron(rup.ind, jy - 4, jz) +
             22. * T_electron(rup.ind, jy - 5, jz)) /
            48. / coord->dy(rup.ind, jy - 1) /
            sqrt((coord->g_22[rup.ind][jy - 1] + coord->g_22[rup.ind][jy - 2] +
                  coord->g_22[rup.ind][jy - 3] + coord->g_22[rup.ind][jy - 4] +
                  coord->g_22[rup.ind][jy - 5]) /
                 5.);
        if (abs(gradT_driveterm[rup.ind][jy][jz]) > 1.e37 ||
            gradT_driveterm[rup.ind][jy][jz] != gradT_driveterm[rup.ind][jy][jz])
          gradT_driveterm[rup.ind][jy][jz] = 1.e37;
#endif
#ifdef DRIVE_GRADV
// Nothing to be done here: gradV_driveterm is CELL_CENTRE and the guard cell values are
// not used
#endif
#ifdef DRIVE_VEMINUSVI
        if (abs(VeminusVi_driveterm[rup.ind][jy][jz]) > 1.e37 ||
            VeminusVi_driveterm[rup.ind][jy][jz] != VeminusVi_driveterm[rup.ind][jy][jz])
          VeminusVi_driveterm[rup.ind][jy][jz] = 1.e37;
#endif
        if (abs(lambdaC_inverse[rup.ind][jy][jz]) > 1.e37 ||
            lambdaC_inverse[rup.ind][jy][jz] != lambdaC_inverse[rup.ind][jy][jz])
          lambdaC_inverse[rup.ind][jy][jz] = 1.e37;
      }
    }
  ...
  do {
    ...
    calc_index(position);
    interp_coefficients = cubic_spline_inverse_lambdaC.coefficients(position);
    // dimensionless_length_deltas_above[jy] and dimensionless_length_deltas_below[jy] are
    // the deltaz's for the half-step above and below, respectively, the CELL_CENTRE at jy
    // deltaz between position[jy](CELL_CENTRE), where t=0, and position[jyp](CELL_YLOW),
    // where t=0.5

    Coordinates *coord = mesh->coordinates();

    dimensionless_length_deltas_above[position->jx][position->jy][position->jz] =
        coord->dy(position->jx, position->jy) *
        sqrt(coord->g_22(position->jx, position->jy)) *
        (interp_coefficients[0] / 2. + interp_coefficients[1] / 2. / 4. +
         interp_coefficients[2] / 3. / 8. + interp_coefficients[3] / 4. / 16.);
    // deltaz between position[jyp](CELL_YLOW), where t=0.5, and
    // position[jyp](CELL_CENTRE), where t=1
    dimensionless_length_deltas_below[position->jx][position->jyp][position->jz] =
        coord->dy(position->jx, position->jy) *
        sqrt(coord->g_22(position->jx, position->jyp)) *
        (interp_coefficients[0] / 2. + interp_coefficients[1] / 2. * 3. / 4. +
         interp_coefficients[2] / 3. * 7. / 8. + interp_coefficients[3] / 4. * 15. / 16.);
    next_index_y(position);

    do {
      interp_coefficients = cubic_spline_inverse_lambdaC.coefficients(position);
      // deltaz between position[jy](CELL_CENTRE), where t=0, and
      // position[jyp](CELL_YLOW), where t=0.5
      ...
      // position[jyp](CELL_CENTRE), where t=1
      dimensionless_length_deltas_below[position->jx][position->jyp][position->jz] =
          coord->dy(position->jx, position->jyp) *
          sqrt(coord->g_22(position->jx, position->jyp)) *
          (interp_coefficients[0] / 2. + interp_coefficients[1] / 2. * 3. / 4. +
           interp_coefficients[2] / 3. * 7. / 8. +
           interp_coefficients[3] / 4. * 15. / 16.);
      increasing_dimensionless_length[position->jx][position->jyp][position->jz] =
          increasing_dimensionless_length[*position] +
          dimensionless_length_deltas_below[*position] +
          dimensionless_length_deltas_above[*position];
    } while (next_index_y(position));
...
#ifdef CALCULATE_HEATFLUX
  electron_heat_flux += -heatflux_gradT_zerocoeff *
                        interp_to(gradT_driveterm, CELL_YLOW) *
                        gradT_electron; // zero eigenvalue contribution to n^(1,1)/T^1.5
#endif
// viscosity gets no zero eigenvalue contribution
#ifdef CALCULATE_FRICTION
  electron_friction += -friction_gradT_zerocoeff * interp_to(gradT_driveterm, CELL_YLOW) *
                       gradT_electron; // zero eigenvalue contribution
#endif
  cubic_spline_gradT_driveterm.calculate(gradT_driveterm);
...
#ifdef CALCULATE_HEATFLUX
  electron_heat_flux +=
      -heatflux_VeminusVi_zerocoeff * interp_to(VeminusVi_driveterm, CELL_YLOW) * jpar;
#endif
// viscosity gets no zero eigenvalue contribution
#ifdef CALCULATE_FRICTION
  electron_friction +=
      -friction_VeminusVi_zerocoeff * interp_to(VeminusVi_driveterm, CELL_YLOW) * jpar;
#endif
  cubic_spline_VeminusVi_driveterm.calculate(VeminusVi_driveterm);
  ...
  for (int i = 0; i < number_of_negative_eigenvalues; i++) {
#ifdef DRIVE_GRADT
    integration.calculateIntegralBelow_cell_ylow(
        eigenvalues[i], dimensionless_length_deltas_below,
        dimensionless_length_deltas_above, cubic_spline_inverse_lambdaC,
        cubic_spline_gradT_driveterm, cubic_spline_gradT, i + 0);
#ifdef CALCULATE_HEATFLUX
    electron_heat_flux += heatflux_gradT_coefficients[i] * integration.integral_below;
#endif
#ifdef CALCULATE_VISCOSITY
    ...
    integration.calculateIntegralBelow_cell_ylow(
        eigenvalues[i], dimensionless_length_deltas_below,
        dimensionless_length_deltas_above, cubic_spline_inverse_lambdaC,
        cubic_spline_gradV_driveterm, cubic_spline_one,
        i + number_of_negative_eigenvalues);
#ifdef CALCULATE_HEATFLUX
    electron_heat_flux += heatflux_gradV_coefficients[i] * integration.integral_below;
#endif
#ifdef CALCULATE_VISCOSITY
    ...
    integration.calculateIntegralBelow_cell_ylow(
        eigenvalues[i], dimensionless_length_deltas_below,
        dimensionless_length_deltas_above, cubic_spline_inverse_lambdaC,
        cubic_spline_VeminusVi_driveterm, cubic_spline_jpar,
        i + 2 * number_of_negative_eigenvalues); // The driveterm here is CELL_YLOW while
                                                 // there is no CELL_CENTRE quantity
                                                 // mulitplying it, so use a cubic spline
                                                 // of a constant field with value 1.
                                                 // everywhere instead (would be slightly
                                                 // more efficient to write a different
                                                 // calculateIntegralBelow_cell_ylow, but
                                                 // tedious).
#ifdef CALCULATE_HEATFLUX
    electron_heat_flux += heatflux_VeminusVi_coefficients[i] * integration.integral_below;
#endif
  ...
  for (int i = 0; i < number_of_negative_eigenvalues; i++) {
#ifdef DRIVE_GRADT
    integration.calculateIntegralAbove_cell_ylow(
        eigenvalues[i], dimensionless_length_deltas_below,
        dimensionless_length_deltas_above, cubic_spline_inverse_lambdaC,
        cubic_spline_gradT_driveterm, cubic_spline_gradT, i + 0);
#ifdef CALCULATE_HEATFLUX
...
#ifdef CALCULATE_VISCOSITY
    electron_viscosity +=
        -viscosity_gradT_coefficients[i] *
        integration.integral_above; // gives \hat(n)^A //gives \hat(n)^A/(T_electron^1.5)
#endif
#ifdef CALCULATE_FRICTION
    electron_friction +=
        -friction_gradT_coefficients[i] *
        integration.integral_above; // gives \hat(n)^A //gives \hat(n)^A/(T_electron^1.5)
#endif
#endif
#ifdef DRIVE_GRADV
...
#ifdef CALCULATE_HEATFLUX
    electron_heat_flux +=
        -heatflux_gradV_coefficients[i] *
        integration.integral_above; // gives \hat(n)^A //gives \hat(n)^A/(T_electron^1.5)
#endif
#ifdef CALCULATE_VISCOSITY
    electron_viscosity +=
        -viscosity_gradV_coefficients[i] *
        integration.integral_above; // gives \hat(n)^A //gives \hat(n)^A/(T_electron^1.5)
#endif
#ifdef CALCULATE_FRICTION
    electron_friction +=
        -friction_gradV_coefficients[i] *
        integration.integral_above; // gives \hat(n)^A //gives \hat(n)^A/(T_electron^1.5)
#endif
#endif
#ifdef DRIVE_VEMINUSVI
    integration.calculateIntegralAbove_cell_ylow(
        ...
        i + 2 * number_of_negative_eigenvalues); // The driveterm here is CELL_YLOW while
                                                 ...
                                                 // everywhere instead (would be slightly
                                                 // more efficient to write a different
                                                 // calculateIntegralAbove_cell_ylow, but
                                                 // tedious).
#ifdef CALCULATE_HEATFLUX
    electron_heat_flux +=
        -heatflux_VeminusVi_coefficients[i] *
        integration.integral_above; // gives \hat(n)^A //gives \hat(n)^A/(T_electron^1.5)
#endif
#ifdef CALCULATE_VISCOSITY
    electron_viscosity +=
        -viscosity_VeminusVi_coefficients[i] *
        integration.integral_above; // gives \hat(n)^A //gives \hat(n)^A/(T_electron^1.5)
#endif
#ifdef CALCULATE_FRICTION
    electron_friction +=
        -friction_VeminusVi_coefficients[i] *
        integration.integral_above; // gives \hat(n)^A //gives \hat(n)^A/(T_electron^1.5)
#endif
#endif
  }
  ...
  electron_heat_flux *=
      -5. / 4. * sqrt(2. / electron_mass) *
      interp_to(T_electron ^ 1.5,
                CELL_YLOW); // now we have q=-5/4*v_Telectron*T_electron*n^(1,1)
  mesh->communicate(electron_heat_flux);
#endif
#ifdef CALCULATE_VISCOSITY
  electron_viscosity *= T_electron;
...
void NonLocalParallel::set_boundary_gradients() {
  for (RangeIterator rlow = mesh->iterateBndryLowerY(); !rlow.isDone(); rlow++)
    for (int jz = 0; jz < mesh->LocalNz; jz++) {
      ...
      //       (electron_heat_flux[rlow.ind][mesh->ystart][jz]-27.*electron_heat_flux[rlow.ind][mesh->ystart+1][jz]+27.*electron_heat_flux[rlow.ind][mesh->ystart+2][jz]-electron_heat_flux[rlow.ind][mesh->ystart+3][jz])/24.;
      //       // NB gradient in index space
      //       BoutReal heat_flux_boundarygradient =
      //       (-11.*electron_heat_flux[rlow.ind][mesh->ystart][jz] +
      //       18.*electron_heat_flux[rlow.ind][mesh->ystart+1][jz] -
      //       9.*electron_heat_flux[rlow.ind][mesh->ystart+2][jz] +
      //       2.*electron_heat_flux[rlow.ind][mesh->ystart+3][jz]) / 6. /
      //       coord->dy[rlow.ind][mesh->ystart] /
      //       sqrt((coord->g_22[rlow.ind][mesh->ystart]+coord->g_22[rlow.ind][mesh->ystart+1]+coord->g_22[rlow.ind][mesh->ystart+2]+coord->g_22[rlow.ind][mesh->ystart+3])/4.);
      BoutReal heat_flux_boundarygradient =
          (-electron_heat_flux[rlow.ind][mesh->ystart][jz] +
           electron_heat_flux[rlow.ind][mesh->ystart + boundary_gradient_smoothing_length]
                             [jz]) /
          BoutReal(boundary_gradient_smoothing_length); // NB gradient in index space
#endif
#ifdef CALCULATE_VISCOSITY
      //       BoutReal viscosity_boundarygradient =
      //       (electron_viscosity[rlow.ind][mesh->ystart][jz]-27.*electron_viscosity[rlow.ind][mesh->ystart+1][jz]+27.*electron_viscosity[rlow.ind][mesh->ystart+2][jz]-electron_viscosity[rlow.ind][mesh->ystart+3][jz])/24.;
      //       // NB gradient in index space
      //       BoutReal viscosity_boundarygradient =
      //       (-11.*electron_viscosity[rlow.ind][mesh->ystart][jz] +
      //       18.*electron_viscosity[rlow.ind][mesh->ystart+1][jz] -
      //       9.*electron_viscosity[rlow.ind][mesh->ystart+2][jz] +
      //       2.*electron_viscosity[rlow.ind][mesh->ystart+3][jz]) / 6. /
      //       coord->dy[rlow.ind][mesh->ystart] /
      //       sqrt((coord->g_22[rlow.ind][mesh->ystart]+coord->g_22[rlow.ind][mesh->ystart+1]+coord->g_22[rlow.ind][mesh->ystart+2]+coord->g_22[rlow.ind][mesh->ystart+3])/4.);
      BoutReal viscosity_boundarygradient =
          (-electron_viscosity[rlow.ind][mesh->ystart][jz] +
           electron_viscosity[rlow.ind][mesh->ystart + boundary_gradient_smoothing_length]
                             [jz]) /
          BoutReal(boundary_gradient_smoothing_length); // NB gradient in index space
#endif
#ifdef CALCULATE_FRICTION
      //       BoutReal friction_boundarygradient =
      //       (electron_friction[rlow.ind][mesh->ystart][jz]-27.*electron_friction[rlow.ind][mesh->ystart+1][jz]+27.*electron_friction[rlow.ind][mesh->ystart+2][jz]-electron_friction[rlow.ind][mesh->ystart+3][jz])/24.;
      //       // NB gradient in index space
      //       BoutReal friction_boundarygradient =
      //       (-11.*electron_friction[rlow.ind][mesh->ystart][jz] +
      //       18.*electron_friction[rlow.ind][mesh->ystart+1][jz] -
      //       9.*electron_friction[rlow.ind][mesh->ystart+2][jz] +
      //       2.*electron_friction[rlow.ind][mesh->ystart+3][jz]) / 6. /
      //       coord->dy[rlow.ind][mesh->ystart] /
      //       sqrt((coord->g_22[rlow.ind][mesh->ystart]+coord->g_22[rlow.ind][mesh->ystart+1]+coord->g_22[rlow.ind][mesh->ystart+2]+coord->g_22[rlow.ind][mesh->ystart+3])/4.);
      BoutReal friction_boundarygradient =
          (-electron_friction[rlow.ind][mesh->ystart][jz] +
           electron_friction[rlow.ind][mesh->ystart + boundary_gradient_smoothing_length]
                            [jz]) /
          BoutReal(boundary_gradient_smoothing_length); // NB gradient in index space
#endif
      for (int jy = mesh->ystart - 1; jy >= 0; jy--) {
#ifdef CALCULATE_HEATFLUX
        electron_heat_flux[rlow.ind][jy][jz] =
            electron_heat_flux[rlow.ind][jy + 1][jz] - heat_flux_boundarygradient;
#endif
#ifdef CALCULATE_VISCOSITY
        electron_viscosity[rlow.ind][jy][jz] =
            electron_viscosity[rlow.ind][jy + 1][jz] - viscosity_boundarygradient;
#endif
#ifdef CALCULATE_FRICTION
        electron_friction[rlow.ind][jy][jz] =
            electron_friction[rlow.ind][jy + 1][jz] - friction_boundarygradient;
#endif
      }
    }
  for (RangeIterator rup = mesh->iterateBndryUpperY(); !rup.isDone(); rup++)
...
#ifdef CALCULATE_HEATFLUX
      // 	BoutReal heat_flux_boundarygradient =
      // (electron_heat_flux[rup.ind][mesh->yend-3][jz]-27.*electron_heat_flux[rup.ind][mesh->yend-2][jz]+27.*electron_heat_flux[rup.ind][mesh->yend-1][jz]-electron_heat_flux[rup.ind][mesh->yend][jz])/24.;
      // // NB gradient in index space
      //       BoutReal heat_flux_boundarygradient =
      //       (11.*electron_heat_flux[rup.ind][mesh->yend][jz] -
      //       18.*electron_heat_flux[rup.ind][mesh->yend-1][jz] +
      //       9.*electron_heat_flux[rup.ind][mesh->yend-2][jz] -
      //       2.*electron_heat_flux[rup.ind][mesh->yend-3][jz]) / 6. /
      //       coord->dy[rup.ind][mesh->yend] /
      //       sqrt((coord->g_22[rup.ind][mesh->yend]+coord->g_22[rup.ind][mesh->yend-1]+coord->g_22[rup.ind][mesh->yend-2]+coord->g_22[rup.ind][mesh->yend-3])/4.);
      BoutReal heat_flux_boundarygradient =
          (-electron_heat_flux[rup.ind][mesh->yend - boundary_gradient_smoothing_length]
                              [jz] +
           electron_heat_flux[rup.ind][mesh->yend][jz]) /
          BoutReal(boundary_gradient_smoothing_length); // NB gradient in index space
#endif
#ifdef CALCULATE_VISCOSITY
      // 	BoutReal viscosity_boundarygradient =
      // (electron_viscosity[rup.ind][mesh->yend-3][jz]-27.*electron_viscosity[rup.ind][mesh->yend-2][jz]+27.*electron_viscosity[rup.ind][mesh->yend-1][jz]-electron_viscosity[rup.ind][mesh->yend][jz])/24.;
      // // NB gradient in index space
      //       BoutReal viscosity_boundarygradient =
      //       (11.*electron_viscosity[rup.ind][mesh->yend][jz] -
      //       18.*electron_viscosity[rup.ind][mesh->yend-1][jz] +
      //       9.*electron_viscosity[rup.ind][mesh->yend-2][jz] -
      //       2.*electron_viscosity[rup.ind][mesh->yend-3][jz]) / 6. /
      //       coord->dy[rup.ind][mesh->yend] /
      //       sqrt((coord->g_22[rup.ind][mesh->yend]+coord->g_22[rup.ind][mesh->yend-1]+coord->g_22[rup.ind][mesh->yend-2]+coord->g_22[rup.ind][mesh->yend-3])/4.);
      BoutReal viscosity_boundarygradient =
          (-electron_viscosity[rup.ind][mesh->yend - boundary_gradient_smoothing_length]
                              [jz] +
           electron_viscosity[rup.ind][mesh->yend][jz]) /
          BoutReal(boundary_gradient_smoothing_length); // NB gradient in index space
#endif
#ifdef CALCULATE_FRICTION
      // 	BoutReal friction_boundarygradient =
      // (electron_friction[rup.ind][mesh->yend-3][jz]-27.*electron_friction[rup.ind][mesh->yend-2][jz]+27.*electron_friction[rup.ind][mesh->yend-1][jz]-electron_friction[rup.ind][mesh->yend][jz])/24.;
      // // NB gradient in index space
      //       BoutReal friction_boundarygradient =
      //       (11.*electron_friction[rup.ind][mesh->yend][jz] -
      //       18.*electron_friction[rup.ind][mesh->yend-1][jz] +
      //       9.*electron_friction[rup.ind][mesh->yend-2][jz] -
      //       2.*electron_friction[rup.ind][mesh->yend-3][jz]) / 6. /
      //       coord->dy[rup.ind][mesh->yend] /
      //       sqrt((coord->g_22[rup.ind][mesh->yend]+coord->g_22[rup.ind][mesh->yend-1]+coord->g_22[rup.ind][mesh->yend-2]+coord->g_22[rup.ind][mesh->yend-3])/4.);
      BoutReal friction_boundarygradient =
          (-electron_friction[rup.ind][mesh->yend - boundary_gradient_smoothing_length]
                             [jz] +
           electron_friction[rup.ind][mesh->yend][jz]) /
          BoutReal(boundary_gradient_smoothing_length); // NB gradient in index space
#endif
      for (int jy = mesh->yend + 1; jy < mesh->LocalNy; jy++) {
#ifdef CALCULATE_HEATFLUX
        electron_heat_flux[rup.ind][jy][jz] =
            electron_heat_flux[rup.ind][jy - 1][jz] + heat_flux_boundarygradient;
#endif
#ifdef CALCULATE_VISCOSITY
        electron_viscosity[rup.ind][jy][jz] =
            electron_viscosity[rup.ind][jy - 1][jz] + viscosity_boundarygradient;
#endif
#ifdef CALCULATE_FRICTION
        electron_friction[rup.ind][jy][jz] =
            electron_friction[rup.ind][jy - 1][jz] + friction_boundarygradient;
#endif
      }
    }
}
...
void NonLocalParallel::y_broadcast(void *input_buffer, const int &size,
...
void NonLocalParallel::rms_over_y(const Field3D &input_field, FieldPerp &output_field) {
  ...
  tempsum = 0.;
  int ye = mesh->yend;
  if (mesh->StaggerGrids && input_field.getLocation() == CELL_CENTRE)
    ye--;
  for (int jx = mesh->xstart; jx <= mesh->xend; jx++)
    for (int jz = 0; jz < mesh->LocalNz; jz++)
  ...
  MPI_Reduce(*tempsum.getData(), *output_field.getData(), mesh->LocalNx * mesh->LocalNz,
             MPI_DOUBLE, MPI_SUM,
  ...
  if (mesh->getYProcIndex() == 0) {
    ...
    mesh->sendToProc(mesh->getXProcIndex(), mesh->getNYPE() - 1, *output_field.getData(),
                     mesh->LocalNx * mesh->LocalNz, NONLOCAL_PARALLEL_TAGBASE);
  } else if (mesh->getYProcIndex() == mesh->getNYPE() - 1) {
    mesh->wait(mesh->receiveFromProc(mesh->getXProcIndex(), 0, *output_field.getData(),
                                     mesh->LocalNx * mesh->LocalNz,
 ...
 */
void NonLocalParallel::mean_over_y(const Field3D &input_field, FieldPerp &output_field,
                                   int exclude_edgecells) {
  FieldPerp tempsum;
  tempsum = 0.;
  int ys = mesh->ystart + exclude_edgecells;
  int ye = mesh->yend - exclude_edgecells;

  if (mesh->StaggerGrids && input_field.getLocation() == CELL_CENTRE && mesh->lastY())
    ye--;
...
BoutReal NonLocalParallel::interp_to_point_YLOW(const Field3D &input, bindex &position) {
  if (mesh->StaggerGrids)
    return (9. * (input(position.jx, position.jym, position.jz) +
                  input(position.jx, position.jy, position.jz)) -
            (input(position.jx, position.jy2m, position.jz) +
             input(position.jx, position.jyp, position.jz))) /
           16.;
  else
