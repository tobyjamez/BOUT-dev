 ...
 * Basically the same as Hazeltine-Meiss but different normalisations and have gravity
 *intead of curvature.
 * Evolving Vorticity U, Parallel electric field Psi, Parallel velocity Vpar, Pressure p,
 *and density rho.
 * Have included compressional terms in Vpar and in pressure and density evolution
 *equations.
 *******************************************************************************/
...
private:
  ...
  int rhs(BoutReal t) override {
    //   U = Delp2(phi);
    phi = invert_laplace(
        U, phi_flags); // Invert Laplacian, setting boundary condition in phi_flags
    phi.applyBoundary();
    ...
    ddt(p) += -((Gamma * p0) / (1 + Gamma * p0 * mu_0 / SQ(B0))) *
              ((rho0 * mu_0 / SQ(B0)) * bracket(G, phi, bm) * coord->Bxy +
               Grad_par_LtoC(Vpar) - (Vpar / B0) * Grad_par(B0));
    ...
    ddt(rho) -=
        (rho0 / (1 + Gamma * p0 * mu_0 / SQ(B0))) *
        ((rho0 * mu_0 / SQ(B0)) * bracket(G, phi, bm) * coord->Bxy + Grad_par_LtoC(Vpar) -
         bracket(Psi, Vpar, bm) * coord->Bxy - (Vpar / B0) * Grad_par(B0));
