/**************************************************************************
 * Testing Perpendicular Laplacian inversion using multigrid solver
 *
 **************************************************************************
...
int main(int argc, char **argv) {
  ...
  // Solving equations of the form d*Grad_perp2(f) + 1/c*Grad_perp(c).Grad_perp(f) + a*f =
  // b for various boundary conditions
  ...
  try {
    ...
    checkData(sol1);
    bcheck1 = d1 * this_Grad_perp2(sol1) + this_Grad_perp_dot_Grad_perp(c1, sol1) / c1 +
              a1 * sol1;
  ...
  try {
    ...
    bcheck2 = d2 * this_Grad_perp2(sol2) + this_Grad_perp_dot_Grad_perp(c2, sol2) / c2 +
              a2 * sol2;
  ...
  try {
    ...
    bcheck3 = d3 * this_Grad_perp2(sol3) + this_Grad_perp_dot_Grad_perp(c3, f3) / c3 +
              a3 * sol3;
  ...
  try {
    ...
    bcheck4 = d4 * this_Grad_perp2(sol4) + this_Grad_perp_dot_Grad_perp(c4, sol4) / c4 +
              a4 * sol4;
...
// Need custom version of perpendicular Laplacian operator, which is the inverse of the
// multigrid solver
...
Field3D this_Grad_perp2(const Field3D &f) {
  Field3D result =
      mesh->coordinates()->G1 * ::DDX(f) + mesh->coordinates()->G3 * ::DDZ(f) +
...
Field3D this_Grad_perp_dot_Grad_perp(const Field3D &f, const Field3D &g) {
  Field3D result = mesh->coordinates()->g11 * ::DDX(f) * ::DDX(g) +
                   mesh->coordinates()->g33 * ::DDZ(f) * ::DDZ(g) +
