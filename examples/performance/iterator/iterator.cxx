...
int main(int argc, char **argv) {
  ...
  ITERATOR_TEST_BLOCK("C loop",
                      for (int j = 0; j < len; ++j) { rd[j] = ad[j] + bd[j]; };);
#ifdef _OPENMP
  ...
  ITERATOR_TEST_BLOCK("Nested loop", for (int i = 0; i < mesh->LocalNx; ++i) {
  ...
  ITERATOR_TEST_BLOCK(
      "DI begin/end",
      for (DataIterator i = std::begin(result), rend = std::end(result); i != rend;
           ++i) { result(i.x, i.y, i.z) = a(i.x, i.y, i.z) + b(i.x, i.y, i.z); });
  ...
  ITERATOR_TEST_BLOCK("C++11 range-based for", for (auto i
                                                    : result) {
    result(i.x, i.y, i.z) = a(i.x, i.y, i.z) + b(i.x, i.y, i.z);
  });
  ...
  ITERATOR_TEST_BLOCK(
      "Region (serial)",
      BLOCK_REGION_LOOP_SERIAL(mesh->getRegion("RGN_ALL"), i, result[i] = a[i] + b[i];););
#ifdef _OPENMP
  ITERATOR_TEST_BLOCK("Region (omp)", BLOCK_REGION_LOOP(mesh->getRegion("RGN_ALL"), i,
                                                        result[i] = a[i] + b[i];););
#endif
