...
MultigridAlg::MultigridAlg(int level, int lx, int lz, int gx, int gz, MPI_Comm comm,
  ...
  if (pcheck > 0)
    output << "Construct MG " << level << endl;
...
void MultigridAlg::getSolution(BoutReal *x, BoutReal *b, int flag) {
  ...
  if (flag == 0) {
    // Solve exaclty
    if (mglevel == 1)
      pGMRES(x, b, mglevel - 1, 1);
    else if (mgplag == 1)
      pGMRES(x, b, mglevel - 1, 1);
    else
      solveMG(x, b, mglevel - 1);
  } else {
    ...
    for (int i = 1; i < lnx[level] + 1; i++)
      for (int k = 1; k < lnz[level] + 1; k++) {
        ...
        BoutReal val = b[nn] - matmg[level][nn * 9 + 3] * x[nn - 1] -
                       matmg[level][nn * 9 + 5] * x[nn + 1] -
                       matmg[level][nn * 9 + 1] * x[nn - mm] -
                       matmg[level][nn * 9 + 7] * x[nn + mm] -
                       matmg[level][nn * 9] * x[nn - mm - 1] -
                       matmg[level][nn * 9 + 2] * x[nn - mm + 1] -
                       matmg[level][nn * 9 + 6] * x[nn + mm - 1] -
...
void MultigridAlg::pGMRES(BoutReal *sol, BoutReal *rhs, int level, int iplag) {
  ...
  BoutReal **v;
  BoutReal c[MAXGM + 1], s[MAXGM + 1], y[MAXGM + 1], g[MAXGM + 1],
      h[MAXGM + 1][MAXGM + 1];
...
do {
  ...
  if (fabs(a1) < atol * rtol) {
    output << num << " First a1 in GMRES is wrong at level " << level << ": " << a1
           << endl;
  }
  a0 = 1.0 / a1;
  g[0] = a1;
...
for (int i = 0; i < it + 1; i++)
  h[i][it] = vectorProd(level, v[it + 1], v[i]);
for (int i = 0; i < it + 1; i++) {
  a0 = -h[i][it];
...
if (fabs(a1) < atol * rtol && ldim > 9) {
  output << num << " Second a1 in GMRES is wrong at level " << level << ": " << a1
         << endl;
}
a0 = 1.0 / a1;
h[it + 1][it] = a1;
...
void MultigridAlg::residualVec(int level, BoutReal *x, BoutReal *b, BoutReal *r) {
...
for (int i = 1; i < lnx[level] + 1; i++)
  for (int k = 1; k < lnz[level] + 1; k++) {
    int nn = i * mm + k;
    BoutReal val =
        matmg[level][nn * 9 + 4] * x[nn] + matmg[level][nn * 9 + 3] * x[nn - 1] +
        matmg[level][nn * 9 + 5] * x[nn + 1] + matmg[level][nn * 9 + 1] * x[nn - mm] +
...
void MultigridAlg::communications(BoutReal *x, int level) {
  ...
  if (zNP > 1) {
    ...
    ierr = MPI_Sendrecv(&x[lnz[level] + 3], 1, xvector, zProcM, stag,
                        &x[2 * (lnz[level] + 2) - 1], 1, xvector, zProcP, rtag, commMG,
                        status);
    ASSERT1(ierr == MPI_SUCCESS);
  ...
  if (xNP > 1) {
    ...
    stag = rProcI;
    rtag = xProcM;
    ierr = MPI_Sendrecv(&x[lnx[level] * (lnz[level] + 2)], lnz[level] + 2, MPI_DOUBLE,
                        xProcP, stag, &x[0], lnz[level] + 2, MPI_DOUBLE, xProcM, rtag,
                        commMG, status);
    ASSERT1(ierr == MPI_SUCCESS);
