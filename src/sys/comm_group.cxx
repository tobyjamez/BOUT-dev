...
bool Comm_gather_start(void *local, int nlocal, MPI_Datatype type, void *data, int root,
                       MPI_Comm comm, Comm_handle_t *handle) {
  ...
  if (!nonblock) {
    ...
    MPI_Gather(local, nlocal, type, data, nlocal, type, root, comm);
  ...
  if (root == handle->myrank) {
    ...
    for (int p = 0; p < handle->nprocs; p++) {
      if (p != root) {
        MPI_Irecv((void *)(((char *)data) + p * nlocal * type_size), nlocal, type, p,
                  COMM_GROUP_TAG, comm, r);
        r++; // Next request
      }
    }

    // Copy local data into output
    memcpy((void *)(((char *)data) + handle->myrank * nlocal * type_size), local,
           nlocal * type_size);
  } else {
    ...
    MPI_Isend(local, nlocal, type, root, COMM_GROUP_TAG, comm, handle->request);
  }
 ...
 */
bool Comm_scatter_start(void *sendbuf, int sendcnt, MPI_Datatype type, void *recvbuf,
                        int root, MPI_Comm comm, Comm_handle_t *handle) {
  ...
  if (root == handle->myrank) {
    ...
    for (int p = 0; p < handle->nprocs; p++) {
      if (p != root) {
        ...
        MPI_Isend((void *)(((char *)sendbuf) + p * sendcnt * type_size), sendcnt, type, p,
                  COMM_GROUP_TAG, comm, r);
        r++; // Next request
      }
    }
    ...
    // Copy local data
    memcpy(recvbuf, (void *)(((char *)sendbuf) + root * sendcnt * type_size),
           sendcnt * type_size);
  } else {
    ...
    MPI_Irecv(recvbuf, sendcnt, type, root, COMM_GROUP_TAG, comm, handle->request);
  }
...
bool Comm_wait(Comm_handle_t *handle) {
  ...
  MPI_Waitall(handle->nreq, handle->request, s);
