...
bool PncFormat::openr(const char *name) {
  ...
  ret = ncmpi_open(comm, name, NC_NOWRITE, MPI_INFO_NULL, &ncfile);
  CHKERR(ret);
...
bool PncFormat::openw(const char *name, bool append) {
  ...
  if (append) {
    ret = ncmpi_open(comm, name, NC_WRITE, MPI_INFO_NULL, &ncfile);
    CHKERR(ret);
    ...
    MPI_Offset len;
    ret = ncmpi_inq_dimlen(ncfile, xDim, &len);
    CHKERR(ret);
    ...
    ret = ncmpi_inq_dimlen(ncfile, yDim, &len);
    CHKERR(ret);
    ...
    ret = ncmpi_inq_dimlen(ncfile, zDim, &len);
    CHKERR(ret);
    ...
    ret = ncmpi_inq_dimlen(ncfile, tDim, &len);
    CHKERR(ret);
    default_rec = (int)len;

  } else {
    /// Create file using collective operation
    ret = ncmpi_create(comm, name, NC_WRITE | NC_64BIT_OFFSET, MPI_INFO_NULL, &ncfile);
    CHKERR(ret);

    /// Add the dimensions

    ret = ncmpi_def_dim(ncfile, "x", mesh->GlobalNx, &xDim);
    CHKERR(ret);

    ret = ncmpi_def_dim(ncfile, "y", mesh->GlobalNy, &yDim);
    CHKERR(ret);

    ret = ncmpi_def_dim(ncfile, "z", mesh->GlobalNz, &zDim);
    CHKERR(ret);

    ret = ncmpi_def_dim(ncfile, "t", NC_UNLIMITED, &tDim);
    CHKERR(ret); // Unlimited
...
const vector<int> PncFormat::getSize(const char *name) {
  ...
  ret = ncmpi_inq_vardimid(ncfile, var, dimid);
  CHKERR(ret);

  for (int i = 0; i < nd; i++) {
    // Get length of each dimension
    MPI_Offset len;
    ret = ncmpi_inq_dimlen(ncfile, dimid[i], &len);
    CHKERR(ret);
...
// Variable not in file
  ...
  // Number of dimensions
  int nd;
  ret = ncmpi_inq_varndims(ncfile, var, &nd);
  CHKERR(ret);

  if (nd == 0) {
    ret = ncmpi_get_var_int_all(ncfile, var, data);
    CHKERR(ret);
    return true;
  }
...
int pnc_get_vara_all(int ncfile, int var, MPI_Offset *start, MPI_Offset *count,
                     double *data) {
...
// Variable not in file
  ...
  if (nd == 0) {
    ret = pnc_get_var_all(ncfile, var, data);
    CHKERR(ret);
    return true;
  }
  ...
  ret = pnc_get_vara_all(ncfile, var, start, count, data);
  CHKERR(ret);
...
bool PncFormat::write(int *data, const char *name, int lx, int ly, int lz) {
  ...
  if (ret = ncmpi_inq_varid(ncfile, name, &var)) {
    ...
    CHKERR(ret);
    // Define variable
    ret = ncmpi_def_var(ncfile, name, NC_INT, nd, dimList, &var);
    CHKERR(ret);
  ...
  if (nd == 0) {
    // Writing a scalar
    ret = ncmpi_put_var_int_all(ncfile, var, data);
    CHKERR(ret);
    return true;
  }
  ...
  ret = ncmpi_put_vara_int_all(ncfile, var, start, count, data);
  CHKERR(ret);
...
// Helper functions to select the correct ncmpi function
int pnc_put_vara_all(int ncfile, int var, MPI_Offset *start, MPI_Offset *count,
                     double *data) {
...
bool PncFormat::write(BoutReal *data, const char *name, int lx, int ly, int lz) {
  ...
  if (ret = ncmpi_inq_varid(ncfile, name, &var)) {
    ...
    // Define variable
    ret = ncmpi_def_var(ncfile, name, type, nd, dimList, &var);
    CHKERR(ret);
    // Back out of define mode
    ret = ncmpi_enddef(ncfile);
    CHKERR(ret);
  ...
  if (nd == 0) {
    // Writing a scalar
    ret = pnc_put_var_all(ncfile, var, data);
    CHKERR(ret);
    return true;
  }
  ...
  ret = pnc_put_vara_all(ncfile, var, start, count, data);
  CHKERR(ret);
...
bool PncFormat::write_rec(int *data, const char *name, int lx, int ly, int lz) {
  ...
  if (ret = ncmpi_inq_varid(ncfile, name, &var)) {
    ...
    CHKERR(ret);
    // Define variable
    ret = ncmpi_def_var(ncfile, name, NC_INT, nd, recDimList, &var);
    CHKERR(ret);
    // Back out of define mode
    ret = ncmpi_enddef(ncfile);
    CHKERR(ret);
...
bool PncFormat::write_rec(BoutReal *data, const char *name, int lx, int ly, int lz) {
  ...
  if (ret = ncmpi_inq_varid(ncfile, name, &var)) {
    ...
    // Define variable
    nc_type type = (lowPrecision) ? NC_FLOAT : NC_DOUBLE;
    ret = ncmpi_def_var(ncfile, name, type, nd, recDimList, &var);
    CHKERR(ret);
    // Back out of define mode
    ret = ncmpi_enddef(ncfile);
    CHKERR(ret);
