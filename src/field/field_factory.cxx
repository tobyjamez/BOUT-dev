...
FieldFactory::FieldFactory(Mesh *localmesh, Options *opt)
    : fieldmesh(localmesh), options(opt) {
  ...
  addGenerator("power",
               std::shared_ptr<FieldGenerator>(new FieldGenTwoArg<pow>(NULL, NULL)));

  addGenerator("round", std::shared_ptr<FieldGenerator>(new FieldRound(NULL)));

  // Ballooning transform
  addGenerator("ballooning",
               std::shared_ptr<FieldGenerator>(new FieldBallooning(fieldmesh)));
  ...
  addGenerator("tanhhat",
               std::shared_ptr<FieldGenerator>(new FieldTanhHat(NULL, NULL, NULL, NULL)));
}

FieldFactory::~FieldFactory() {}

const Field2D FieldFactory::create2D(const string &value, Options *opt, Mesh *localmesh,
                                     CELL_LOC loc, BoutReal t) {
  ...
  if (!gen) {
    output << "FieldFactory error: Couldn't create 2D field from '" << value << "'"
           << endl;
    return result;
  }
  ...
  default: { // CELL_CENTRE or CELL_ZLOW
    for (auto i : result) {
      result[i] = gen->generate(localmesh->GlobalX(i.x), TWOPI * localmesh->GlobalY(i.y),
                                0.0, // Z
                                t);  // T
    }
  }
...
const Field3D FieldFactory::create3D(const string &value, Options *opt, Mesh *localmesh,
                                     CELL_LOC loc, BoutReal t) {
  ...
  std::shared_ptr<FieldGenerator> gen = parse(value, opt);
  if (!gen) {
    throw BoutException("FieldFactory error: Couldn't create 3D field from '%s'",
                        value.c_str());
  }
