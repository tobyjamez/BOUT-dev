...
int physics_init(bool restarting) {
  ...
  BLOCK_REGION_LOOP(reg, i, a[i] = 3.0; b[i] = c[i];);
  ...
  BLOCK_REGION_LOOP(mesh->getRegion3D("RGN_NOBNDRY"), i, d[i] = 3.0; e[i] = f[i];);

  // Check expected results
  nerr = 0;
  // Expect to find differences just in the boundaries, so work out how many boundary
  // points there area
  const int nerrExpected = (2 * mesh->xstart * mesh->LocalNy +
