...
public:
  LaplacePetsc(Options *UNUSED(opt) = NULL) {
...
private:
  ...
  // Metrics are not constant in y-direction, so matrix always changes as you loop over
  // the grid
  // Hence using coefchanged switch to avoid recomputing the mmatrix is not a useful thing
  // to do (unless maybe in a cylindrical machine, but not worth implementing just for
  // that)
  //   bool coefchanged;           // Set to true when C, D, Ex or Ez coefficients are
  //   changed
  ...
  // Convergence Parameters. Solution is considered converged if |r_k| < max( rtol * |b| ,
  // atol )
