...
// Create an error object so netCDF doesn't exit
  ...
  if (append) {
    ...
    if ((xDim->size() != mesh->LocalNx) || (yDim->size() != mesh->LocalNy) ||
        (zDim->size() != mesh->LocalNz) || (!tDim->is_unlimited())) {
      delete dataFile;
      dataFile = NULL;
      return false;
    }
...
const vector<int> NcFormat::getSize(const string &var) { return getSize(var.c_str()); }
