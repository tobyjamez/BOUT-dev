...
public:
  ...
  bool dataIsLocal() const {
    return valid() && (data_on_proc == mype);
...
protected:
  ...
  void proc_origin(int proc, int *x, int *y,
                   int *z = NULL) const; ///< Return the global origin of processor proc
  void proc_size(int proc, int *lx, int *ly,
                 int *lz = NULL) const; ///< Return the array size of processor proc
private:
  GlobalField();
};
...
public:
  ...
  /// @param[in] proc   The processor index where everything will be gathered/scattered
  /// to/from
  ...
  /// Assignment from a 2D field. Shorthand for a gather, and must be called on all
  /// processors
  ...
  const BoutReal &operator()(int jx, int jy) const {
    return GlobalField::operator()(jx, jy, 0);
  }
