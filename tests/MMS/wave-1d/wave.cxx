  ...
  for (int i = mesh->xstart; i <= mesh->xend; i++)
    for (int j = mesh->ystart; j <= mesh->yend; j++)
      for (int k = 0; k < mesh->LocalNz; k++) {
        ...
        BoutReal fp = (SR * f(i, j, k) - SL * f(i + 1, j, k) +
                       SL * SR * (u(i + 1, j, k) - u(i, j, k))) /
                      (SR - SL);
        BoutReal fm = (SR * f(i - 1, j, k) - SL * f(i, j, k) +
                       SL * SR * (u(i, j, k) - u(i - 1, j, k))) /
                      (SR - SL);
      ...
      for (int xi = mesh->xstart; xi < mesh->xend + 1; xi++) {
        for (int yj = mesh->ystart; yj < mesh->yend + 1; yj++) {
          for (int zk = 0; zk < mesh->LocalNz; zk++) {
            f(xi, yj, zk) =
                MS_f(0., mesh->GlobalX(xi), mesh->GlobalY(yj), coord->dz * zk);
            g(xi, yj, zk) = MS_g(0., 0.5 * (mesh->GlobalX(xi) + mesh->GlobalX(xi - 1)),
                                 mesh->GlobalY(yj), coord->dz * zk);
          }
        }
      }
    } else {
        ...
        for (int yj = mesh->ystart; yj < mesh->yend + 1; yj++) {
          for (int zk = 0; zk < mesh->LocalNz; zk++) {
            f(xi, yj, zk) =
                MS_f(0., mesh->GlobalX(xi), mesh->GlobalY(yj), coord->dz * zk);
            g(xi, yj, zk) =
                MS_g(0., mesh->GlobalX(xi), mesh->GlobalY(yj), coord->dz * zk);
          }
        }
      }
    }
    ...
    for (int xi = mesh->xstart; xi < mesh->xend + 1; xi++) {
      for (int yj = mesh->ystart; yj < mesh->yend + 1; yj++) {
        for (int zk = 0; zk < mesh->LocalNz; zk++) {
          ...
          output_error.write("Error at %d,%d,%d = %e, %e", xi, yj, zk, E_f(xi, yj, zk),
                             E_g(xi, yj, zk));
        }
      }
    }
  ...
  for (xi = mesh->xstart; xi < mesh->xend + 1; xi++)
    for (yj = mesh->ystart; yj < mesh->yend + 1; yj++) {
      for (zk = 0; zk < mesh->LocalNz; zk++) {
        ...
        z = zk * coord->dz;
        result(xi, yj, zk) = -0.8 * x * Cos(7 * t) * Cos(2.0 * Power(x, 2)) -
                             2.0 * Sin(10 * t) * Sin(5.0 * Power(x, 2)) - 0.7;
      }
    }
  return result;
}
  ...
  for (xi = mesh->xstart; xi < mesh->xend + 1; xi++)
    for (yj = mesh->ystart; yj < mesh->yend + 1; yj++) {
      for (zk = 0; zk < mesh->LocalNz; zk++) {
        ...
        y = mesh->GlobalY(yj) * Ly;
        z = zk * coord->dz;
        result(xi, yj, zk) = -2.0 * x * Cos(10 * t) * Cos(5.0 * Power(x, 2)) -
                             1.4 * Sin(7 * t) * Sin(2.0 * Power(x, 2)) - 0.9;
      }
    }
  return result;
}
