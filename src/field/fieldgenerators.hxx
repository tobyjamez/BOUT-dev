  ...
  clone(const list<std::shared_ptr<FieldGenerator>> args) {
    if (args.size() != 1) {
      throw ParseException(
          "Incorrect number of arguments to function. Expecting 1, got %d", args.size());
    }
...
public:
  FieldGenTwoArg(std::shared_ptr<FieldGenerator> a, std::shared_ptr<FieldGenerator> b)
      : A(a), B(b) {}
  std::shared_ptr<FieldGenerator>
  clone(const list<std::shared_ptr<FieldGenerator>> args) {
    if (args.size() != 2) {
    ...
    return std::shared_ptr<FieldGenerator>(
        new FieldGenTwoArg<Op>(args.front(), args.back()));
  }
  BoutReal generate(double x, double y, double z, double t) {
...
public:
  FieldATan(std::shared_ptr<FieldGenerator> a,
...
public:
  FieldGaussian(std::shared_ptr<FieldGenerator> xin, std::shared_ptr<FieldGenerator> sin)
      : X(xin), s(sin) {}
...
public:
  FieldBallooning(Mesh *m, std::shared_ptr<FieldGenerator> a = nullptr, int n = 3)
