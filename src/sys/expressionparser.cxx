...
public:
  ...
  clone(const list<std::shared_ptr<FieldGenerator>> UNUSED(args)) {
    return std::shared_ptr<FieldGenerator>(new FieldX());
  }
...
public:
  std::shared_ptr<FieldGenerator>
  clone(const list<std::shared_ptr<FieldGenerator>> UNUSED(args)) {
    return std::shared_ptr<FieldGenerator>(new FieldY());
  }
...
public:
  std::shared_ptr<FieldGenerator>
  clone(const list<std::shared_ptr<FieldGenerator>> UNUSED(args)) {
    return std::shared_ptr<FieldGenerator>(new FieldZ());
  }
...
public:
  std::shared_ptr<FieldGenerator>
  clone(const list<std::shared_ptr<FieldGenerator>> UNUSED(args)) {
    return std::shared_ptr<FieldGenerator>(new FieldT());
  }
...
public:
  ...
  std::shared_ptr<FieldGenerator>
  clone(const list<std::shared_ptr<FieldGenerator>> args) {
    if (args.size() != 1) {
      throw ParseException(
          "Incorrect number of arguments to unary minus. Expecting 1, got %d",
          args.size());
    }
...
void ExpressionParser::addBinaryOp(char sym, std::shared_ptr<FieldGenerator> b,
                                   int precedence) {
...
std::shared_ptr<FieldGenerator>
ExpressionParser::parseBinOpRHS(LexInfo &lex, int ExprPrec,
                                std::shared_ptr<FieldGenerator> lhs) {
  ...
  while (true) {
    ...
    map<char, pair<std::shared_ptr<FieldGenerator>, int>>::iterator it =
        bin_op.find(lex.curtok);
...
int ExpressionParser::LexInfo::getPos() { return static_cast<int>(ss.tellg()); }
